#!/usr/bin/env bash
# recipes/trove-classifiers-cp311/build.sh — build trove-classifiers 2026.6.1.19 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311, calver-cp311
...
echo "trove-classifiers-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trove-classifiers-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trove-classifiers-cp311: built $(basename "${WHEEL}")"
