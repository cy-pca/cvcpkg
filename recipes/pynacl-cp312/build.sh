#!/usr/bin/env bash
# recipes/pynacl-cp312/build.sh — build pynacl 1.6.2 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, wheel-cp312, cffi-cp312, pycparser-cp312
...
echo "pynacl-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pynacl-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pynacl-cp312: built $(basename "${WHEEL}")"
