#!/usr/bin/env bash
# recipes/asyncpg-cp313/build.sh — build asyncpg 0.31.0 FROM SOURCE (generated).
#
...
echo "asyncpg-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "asyncpg-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "asyncpg-cp313: built $(basename "${WHEEL}")"
