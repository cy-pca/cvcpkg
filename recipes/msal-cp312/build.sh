#!/usr/bin/env bash
# recipes/msal-cp312/build.sh — build msal 1.37.0 FROM SOURCE (generated).
#
...
echo "msal-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "msal-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "msal-cp312: built $(basename "${WHEEL}")"
