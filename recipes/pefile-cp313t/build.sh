#!/usr/bin/env bash
# recipes/pefile-cp313t/build.sh — build pefile 2024.8.26 FROM SOURCE (generated).
#
...
echo "pefile-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pefile-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pefile-cp313t: built $(basename "${WHEEL}")"
