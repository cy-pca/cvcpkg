#!/usr/bin/env bash
# recipes/sympy-cp313/build.sh — build sympy 1.13.3 FROM SOURCE (generated).
#
...
echo "sympy-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "sympy-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "sympy-cp313: built $(basename "${WHEEL}")"
