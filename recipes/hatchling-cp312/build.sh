#!/usr/bin/env bash
# recipes/hatchling-cp312/build.sh — build hatchling 1.31.0 FROM SOURCE (generated).
#
...
echo "hatchling-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatchling-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatchling-cp312: built $(basename "${WHEEL}")"
