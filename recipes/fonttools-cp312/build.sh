#!/usr/bin/env bash
# recipes/fonttools-cp312/build.sh — build fonttools 4.63.0 FROM SOURCE (generated).
#
...
echo "fonttools-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fonttools-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fonttools-cp312: built $(basename "${WHEEL}")"
