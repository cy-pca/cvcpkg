#!/usr/bin/env bash
# recipes/tinycss2-cp313/build.sh — build tinycss2 1.5.1 FROM SOURCE (generated).
#
...
echo "tinycss2-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tinycss2-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tinycss2-cp313: built $(basename "${WHEEL}")"
