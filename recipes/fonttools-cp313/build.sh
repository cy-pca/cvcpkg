...
echo "fonttools-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fonttools-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fonttools-cp313: built $(basename "${WHEEL}")"
