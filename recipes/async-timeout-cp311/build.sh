#!/usr/bin/env bash
# recipes/async-timeout-cp311/build.sh — build async-timeout 5.0.1 FROM SOURCE (generated).
#
...
echo "async-timeout-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "async-timeout-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "async-timeout-cp311: built $(basename "${WHEEL}")"
