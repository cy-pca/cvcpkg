#!/usr/bin/env bash
# recipes/setuptools-cp311/build.sh — build setuptools 80.9.0 FROM SOURCE (generated).
#
...
echo "setuptools-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "setuptools-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "setuptools-cp311: built $(basename "${WHEEL}")"
