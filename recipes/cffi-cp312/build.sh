#!/usr/bin/env bash
# recipes/cffi-cp312/build.sh — build cffi 2.0.0 FROM SOURCE for the cp312
...
echo "cffi-cp312: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools; print("cffi-cp312: setuptools", setuptools.__version__, setuptools.__file__)'
...
[ -n "${PKG_CONFIG:-}" ] || {
    echo "cffi-cp312: no pkg-config in ${BLD}/bin, ${DEPS}/bin or on PATH" >&2; exit 1; }
...
if ! "${PKG_CONFIG}" --exists libffi; then
    echo "cffi-cp312: libffi.pc not found by ${PKG_CONFIG} — is the libffi dep in the closure?" >&2
    echo "  PKG_CONFIG_LIBDIR=${PKG_CONFIG_LIBDIR}" >&2
    ls -la "${DEPS}/lib/pkgconfig" "${BLD}/lib/pkgconfig" 2>&1 | head -40 >&2
    exit 1
fi
echo "cffi-cp312: libffi $("${PKG_CONFIG}" --modversion libffi) from $("${PKG_CONFIG}" --variable=prefix libffi)"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "cffi-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cffi-cp312: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "cffi-cp312: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "cffi-cp312: staged into ${SITE_PACKAGES}"
...
if [ "${CVC_PLATFORM}" = "macos" ]; then
    ...
    command -v install_name_tool >/dev/null 2>&1 || {
        echo "cffi-cp312: install_name_tool required on macOS but not found" >&2; exit 1; }
    ...
    command -v patchelf >/dev/null 2>&1 || {
        echo "cffi-cp312: patchelf required on ${CVC_PLATFORM} but not on PATH" >&2; exit 1; }
    while IFS= read -r -d '' _so; do
        _rel="$(_relpath_to_lib "${_so}")"
...
echo "cffi-cp312: build + verification complete"
