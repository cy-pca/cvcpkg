#!/usr/bin/env bash
# recipes/packaging-cp312/build.sh — build packaging 24.2 FROM SOURCE (generated).
#
...
echo "packaging-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "packaging-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "packaging-cp312: built $(basename "${WHEEL}")"
