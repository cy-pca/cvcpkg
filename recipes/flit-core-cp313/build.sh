#!/usr/bin/env bash
# recipes/flit-core-cp313/build.sh — build flit-core 3.12.0 FROM SOURCE (generated).
#
...
echo "flit-core-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "flit-core-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "flit-core-cp313: built $(basename "${WHEEL}")"
