#!/usr/bin/env bash
# recipes/charset-normalizer-cp311/build.sh — build charset-normalizer 3.4.9 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311
...
echo "charset-normalizer-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "charset-normalizer-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "charset-normalizer-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import charset_normalizer"
