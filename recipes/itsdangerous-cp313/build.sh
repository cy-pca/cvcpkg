#!/usr/bin/env bash
# recipes/itsdangerous-cp313/build.sh — build itsdangerous 2.2.0 FROM SOURCE (generated).
#
...
echo "itsdangerous-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "itsdangerous-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "itsdangerous-cp313: built $(basename "${WHEEL}")"
