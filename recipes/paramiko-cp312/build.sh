#!/usr/bin/env bash
# recipes/paramiko-cp312/build.sh — build paramiko 5.0.0 FROM SOURCE (generated).
#
...
echo "paramiko-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "paramiko-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "paramiko-cp312: built $(basename "${WHEEL}")"
