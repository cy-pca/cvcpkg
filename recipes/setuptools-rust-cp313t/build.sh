#!/usr/bin/env bash
# recipes/setuptools-rust-cp313t/build.sh — build setuptools_rust 1.13.0 FROM SOURCE (sdist) for the
...
echo "setuptools-rust-cp313t: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, setuptools_scm; print("setuptools-rust-cp313t: setuptools", setuptools.__version__, "+ setuptools_scm")'
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "setuptools-rust-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "setuptools-rust-cp313t: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "setuptools-rust-cp313t: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "setuptools-rust-cp313t: staged into ${SITE_PACKAGES}"
...
echo "setuptools-rust-cp313t: build + verification complete"
