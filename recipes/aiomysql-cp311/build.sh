#!/usr/bin/env bash
# recipes/aiomysql-cp311/build.sh — build aiomysql 0.3.2 FROM SOURCE (generated).
#
...
echo "aiomysql-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "aiomysql-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "aiomysql-cp311: built $(basename "${WHEEL}")"
