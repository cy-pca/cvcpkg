#!/usr/bin/env bash
# recipes/farama-notifications-cp313t/build.sh — build farama-notifications 0.0.6 FROM SOURCE (generated).
#
...
echo "farama-notifications-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "farama-notifications-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "farama-notifications-cp313t: built $(basename "${WHEEL}")"
