#!/usr/bin/env bash
# recipes/poetry-core-cp313t/build.sh — build poetry-core 2.4.1 FROM SOURCE (generated).
#
...
echo "poetry-core-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "poetry-core-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "poetry-core-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import poetry.core"
