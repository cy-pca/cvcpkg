#!/usr/bin/env bash
# recipes/asyncpg-cp312/build.sh — build asyncpg 0.31.0 FROM SOURCE (generated).
#
...
echo "asyncpg-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "asyncpg-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "asyncpg-cp312: built $(basename "${WHEEL}")"
