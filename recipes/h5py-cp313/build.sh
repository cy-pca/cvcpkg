#!/usr/bin/env bash
# recipes/h5py-cp313/build.sh — build h5py 3.16.0 FROM SOURCE against cvcpkg's
# OWN HDF5 (recipes/hdf5, libhdf5.so.310), for the cp313 interpreter column of
...
#   <prefix>/lib/python3.13/site-packages/h5py/*.so
# so the rpath to reach <prefix>/lib is $ORIGIN/../../.. (three up: h5py ->
# site-packages -> python3.13 -> lib). We compute it per-file so the depth is
...
# (cython-cp313, pkgconfig-cp313, wheel-cp313, build-cp313) plus numpy-cp313 and
# the base python313 (pip + setuptools). This script does NOT re-solve that — it
...
# a cp313 default so the script is runnable standalone.
: "${CVC_PYTHON_ABI:=cp313}"
PY_EXE="$(cvc_python_exe)"
echo "h5py-cp313: building against ${PY_EXE}"
...
echo "h5py-cp313: HDF5_DIR=${HDF5_DIR}  HDF5_VERSION=${HDF5_VERSION}"
[ -e "${HDF5_DIR}/include/hdf5.h" ] || {
    echo "h5py-cp313: ${HDF5_DIR}/include/hdf5.h missing — is the hdf5 dep in the closure?" >&2
    exit 1
}
...
[ -n "${WHEEL}" ] || { echo "h5py-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "h5py-cp313: built $(basename "${WHEEL}")"
...
# site-packages scheme (PY_EXE decides the scheme -> lib/python3.13/site-packages).
...
[ -n "${H5PY_DIR}" ] || { echo "h5py-cp313: staged h5py/ not found under ${CVC_INSTALL_DIR}" >&2; exit 1; }
SITE_PACKAGES="$(dirname "${H5PY_DIR}")"
echo "h5py-cp313: staged into ${SITE_PACKAGES}"
...
if [[ "${CVC_PLATFORM}" == "macos" ]]; then
    ...
    command -v install_name_tool >/dev/null 2>&1 || {
        echo "h5py-cp313: install_name_tool required on macOS but not found" >&2; exit 1; }
...
PY
    ...
    if ! command -v patchelf >/dev/null 2>&1; then
        echo "h5py-cp313: patchelf required on ${CVC_PLATFORM} but not found on PATH" >&2
        exit 1
    fi
    while IFS= read -r -d '' _so; do
...
print("h5py-cp313 round-trip OK")
PYCHECK

echo "h5py-cp313: build + verification complete"
