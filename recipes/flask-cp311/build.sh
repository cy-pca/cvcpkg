#!/usr/bin/env bash
# recipes/flask-cp311/build.sh — build flask 3.1.3 FROM SOURCE (generated).
#
...
echo "flask-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "flask-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "flask-cp311: built $(basename "${WHEEL}")"
