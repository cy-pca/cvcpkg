#!/usr/bin/env bash
# recipes/uvicorn-cp313/build.sh — build uvicorn 0.47.0 FROM SOURCE (generated).
#
...
echo "uvicorn-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "uvicorn-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "uvicorn-cp313: built $(basename "${WHEEL}")"
