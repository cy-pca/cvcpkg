#!/usr/bin/env bash
# recipes/certifi-cp312/build.sh — build certifi 2026.5.20 FROM SOURCE (generated).
#
...
echo "certifi-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "certifi-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "certifi-cp312: built $(basename "${WHEEL}")"
