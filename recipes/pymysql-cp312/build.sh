#!/usr/bin/env bash
# recipes/pymysql-cp312/build.sh — build pymysql 1.2.0 FROM SOURCE (generated).
#
...
echo "pymysql-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pymysql-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pymysql-cp312: built $(basename "${WHEEL}")"
