#!/usr/bin/env bash
# recipes/tomli-cp312/build.sh — build tomli 2.4.1 FROM SOURCE (generated).
#
...
echo "tomli-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tomli-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tomli-cp312: built $(basename "${WHEEL}")"
