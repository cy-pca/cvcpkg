#!/usr/bin/env bash
# recipes/python-multipart-cp313/build.sh — build python-multipart 0.0.29 FROM SOURCE (generated).
#
...
echo "python-multipart-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "python-multipart-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "python-multipart-cp313: built $(basename "${WHEEL}")"
