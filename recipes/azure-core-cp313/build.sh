#!/usr/bin/env bash
# recipes/azure-core-cp313/build.sh — build azure-core 1.41.0 FROM SOURCE (generated).
#
...
echo "azure-core-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "azure-core-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "azure-core-cp313: built $(basename "${WHEEL}")"
