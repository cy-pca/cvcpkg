#!/usr/bin/env bash
# recipes/typing-inspection-cp311/build.sh — build typing-inspection 0.4.2 FROM SOURCE (generated).
#
...
echo "typing-inspection-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "typing-inspection-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "typing-inspection-cp311: built $(basename "${WHEEL}")"
