#!/usr/bin/env bash
# recipes/pyphen-cp312/build.sh — build pyphen 0.18.1 FROM SOURCE (generated).
#
...
echo "pyphen-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyphen-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyphen-cp312: built $(basename "${WHEEL}")"
