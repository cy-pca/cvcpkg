#!/usr/bin/env bash
# recipes/urllib3-cp313/build.sh — build urllib3 2.7.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp313, hatch-vcs-cp313, setuptools-scm-cp313, packaging-cp313, pathspec-cp313, pluggy-cp313, trove-classifiers-cp313, calver-cp313
...
echo "urllib3-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "urllib3-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "urllib3-cp313: built $(basename "${WHEEL}")"
