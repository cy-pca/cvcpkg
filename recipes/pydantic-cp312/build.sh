#!/usr/bin/env bash
# recipes/pydantic-cp312/build.sh — build pydantic 2.13.4 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp312, hatch-fancy-pypi-readme-cp312, packaging-cp312, pathspec-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "pydantic-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pydantic-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydantic-cp312: built $(basename "${WHEEL}")"
