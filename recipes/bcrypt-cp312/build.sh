#!/usr/bin/env bash
# recipes/bcrypt-cp312/build.sh — build bcrypt 5.0.0 FROM SOURCE for the cp312
...
echo "bcrypt-cp312: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, setuptools_rust, wheel; print("bcrypt-cp312: setuptools", setuptools.__version__, "+ setuptools_rust + wheel")'
...
_require_prefix_tool() {
    ...
    [ -n "${resolved}" ] || {
        echo "bcrypt-cp312: ${tool} not on PATH — is the 'rust' build dep in the closure?" >&2
        exit 1
    }
    case "${resolved}" in
        "${BLD}"/*|"${DEPS}"/*) echo "${resolved}" ;;
        *)
            echo "bcrypt-cp312: refusing ${tool} at ${resolved} — it is outside the cvcpkg" >&2
...
echo "bcrypt-cp312: cargo $("${CARGO_BIN}" --version) [${CARGO_BIN}]"
echo "bcrypt-cp312: rustc $("${RUSTC_BIN}" --version) [${RUSTC_BIN}]"
...
if [ -n "${CVC_CARGO_OFFLINE:-}" ]; then
    ...
    echo "bcrypt-cp312: CARGO_NET_OFFLINE=true — ${CARGO_HOME} must already contain"
    echo "  every crate in Cargo.lock, or cargo will fail rather than fetch."
else
    export CARGO_NET_OFFLINE=false
    echo "bcrypt-cp312: NOTE — cargo will FETCH crates.io. This sdist ships"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "bcrypt-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "bcrypt-cp312: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "bcrypt-cp312: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "bcrypt-cp312: staged into ${SITE_PACKAGES}"
...
echo "bcrypt-cp312: build + verification complete"
