#!/usr/bin/env bash
# recipes/gymnasium-cp311/build.sh — build gymnasium 1.3.0 FROM SOURCE (generated).
#
...
echo "gymnasium-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gymnasium-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gymnasium-cp311: built $(basename "${WHEEL}")"
