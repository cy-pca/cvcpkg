#!/usr/bin/env bash
# recipes/msal-extensions-cp311/build.sh — build msal-extensions 1.3.1 FROM SOURCE (generated).
#
...
echo "msal-extensions-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "msal-extensions-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "msal-extensions-cp311: built $(basename "${WHEEL}")"
