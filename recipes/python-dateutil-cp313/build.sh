#!/usr/bin/env bash
# recipes/python-dateutil-cp313/build.sh — build python-dateutil 2.9.0.post0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, wheel-cp313, setuptools-scm-cp313
...
echo "python-dateutil-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "python-dateutil-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "python-dateutil-cp313: built $(basename "${WHEEL}")"
