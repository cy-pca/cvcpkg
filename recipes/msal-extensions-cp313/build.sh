#!/usr/bin/env bash
# recipes/msal-extensions-cp313/build.sh — build msal-extensions 1.3.1 FROM SOURCE (generated).
#
...
echo "msal-extensions-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "msal-extensions-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "msal-extensions-cp313: built $(basename "${WHEEL}")"
