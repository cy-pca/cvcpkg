#!/usr/bin/env bash
# recipes/jsonschema-cp312/build.sh — build jsonschema 4.17.3 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp312, hatch-vcs-cp312, hatch-fancy-pypi-readme-cp312, packaging-cp312, pathspec-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "jsonschema-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jsonschema-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jsonschema-cp312: built $(basename "${WHEEL}")"
