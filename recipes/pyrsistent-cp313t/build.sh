#!/usr/bin/env bash
# recipes/pyrsistent-cp313t/build.sh — build pyrsistent 0.20.0 FROM SOURCE (generated).
#
...
echo "pyrsistent-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyrsistent-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyrsistent-cp313t: built $(basename "${WHEEL}")"
