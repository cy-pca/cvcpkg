#!/usr/bin/env bash
# recipes/googleapis-common-protos-cp312/build.sh — build googleapis-common-protos 1.75.0 FROM SOURCE (generated).
#
...
echo "googleapis-common-protos-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "googleapis-common-protos-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "googleapis-common-protos-cp312: built $(basename "${WHEEL}")"
