#!/usr/bin/env bash
# recipes/wheel-cp311/build.sh — build wheel 0.45.1 FROM SOURCE (generated).
#
...
echo "wheel-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "wheel-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "wheel-cp311: built $(basename "${WHEEL}")"
