#!/usr/bin/env bash
# recipes/google-auth-cp312/build.sh — build google-auth 2.56.0 FROM SOURCE (generated).
#
...
echo "google-auth-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-auth-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-auth-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import google.auth"
