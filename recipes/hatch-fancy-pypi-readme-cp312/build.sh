#!/usr/bin/env bash
# recipes/hatch-fancy-pypi-readme-cp312/build.sh — build hatch-fancy-pypi-readme 24.1.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp312, packaging-cp312, pathspec-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "hatch-fancy-pypi-readme-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatch-fancy-pypi-readme-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatch-fancy-pypi-readme-cp312: built $(basename "${WHEEL}")"
