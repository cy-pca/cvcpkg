#!/usr/bin/env bash
# recipes/pyyaml-cp311/build.sh — build PyYAML 6.0.3 FROM SOURCE for the cp311
...
echo "pyyaml-cp311: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools; print("pyyaml-cp311: setuptools", setuptools.__version__, setuptools.__file__)'
...
if [ ! -f "${DEPS}/include/yaml.h" ] && [ ! -f "${BLD}/include/yaml.h" ]; then
    echo "pyyaml-cp311: yaml.h not found in ${DEPS}/include or ${BLD}/include" >&2
...
"${PY}" -c 'import Cython; print("pyyaml-cp311: Cython", Cython.__version__)'

PYYAML_CFG="$(printf '{"include_dirs": ["%s", "%s"], "library_dirs": ["%s", "%s"]}' \
    "${DEPS}/include" "${BLD}/include" "${DEPS}/lib" "${BLD}/lib")"
echo "pyyaml-cp311: pyyaml_build_config=${PYYAML_CFG}"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "pyyaml-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyyaml-cp311: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "pyyaml-cp311: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "pyyaml-cp311: staged into ${SITE_PACKAGES}"
...
if [ "${CVC_PLATFORM}" = "macos" ]; then
    ...
    command -v install_name_tool >/dev/null 2>&1 || {
        echo "pyyaml-cp311: install_name_tool required on macOS but not found" >&2; exit 1; }
    ...
    command -v patchelf >/dev/null 2>&1 || {
        echo "pyyaml-cp311: patchelf required on ${CVC_PLATFORM} but not on PATH" >&2; exit 1; }
    while IFS= read -r -d '' _so; do
        _rel="$(_relpath_to_lib "${_so}")"
...
echo "pyyaml-cp311: build + verification complete"
