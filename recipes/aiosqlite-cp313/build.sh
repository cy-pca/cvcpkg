#!/usr/bin/env bash
# recipes/aiosqlite-cp313/build.sh — build aiosqlite 0.22.1 FROM SOURCE (generated).
#
...
echo "aiosqlite-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "aiosqlite-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "aiosqlite-cp313: built $(basename "${WHEEL}")"
