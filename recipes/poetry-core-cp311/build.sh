#!/usr/bin/env bash
# recipes/poetry-core-cp311/build.sh — build poetry-core 2.4.1 FROM SOURCE (generated).
#
...
echo "poetry-core-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "poetry-core-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "poetry-core-cp311: built $(basename "${WHEEL}")"
