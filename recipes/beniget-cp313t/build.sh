#!/usr/bin/env bash
# recipes/beniget-cp313t/build.sh — build beniget 0.5.0 FROM SOURCE (generated).
#
...
echo "beniget-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "beniget-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "beniget-cp313t: built $(basename "${WHEEL}")"
