#!/usr/bin/env bash
# recipes/wheel-cp313/build.sh — build wheel 0.45.1 FROM SOURCE (generated).
#
...
echo "wheel-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "wheel-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "wheel-cp313: built $(basename "${WHEEL}")"
