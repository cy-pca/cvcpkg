#!/usr/bin/env bash
# recipes/filelock-cp311/build.sh — build filelock 3.16.1 FROM SOURCE (generated).
#
...
echo "filelock-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "filelock-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "filelock-cp311: built $(basename "${WHEEL}")"
