#!/usr/bin/env bash
# recipes/pymupdf-cp313/build.sh — build PyMuPDF 1.28.2 FROM SOURCE for the cp313
...
#      ABI OFF (a version-specific cp313 wheel — see PYMUPDF_SETUP_PY_LIMITED_API),
...
PYMM="$(cvc_interp_version "${CVC_PYTHON_INTERPRETER}")"   # e.g. 3.13
echo "pymupdf-cp313: building with ${PY} (python${PYMM})"
...
echo "pymupdf-cp313: downloading ${MUPDF_URL}"
...
if [ "${_mupdf_actual}" != "${MUPDF_SHA256}" ]; then
    echo "pymupdf-cp313: MuPDF source sha256 mismatch" >&2
...
[ -d "${MUPDF_SRC}" ] || { echo "pymupdf-cp313: extracted MuPDF dir ${MUPDF_SRC} missing" >&2; exit 1; }

# ── 2. Py2 -> Py3 fix in MuPDF's SWIG template ──────────────────────────────
_swigpy="${MUPDF_SRC}/scripts/wrap/swig.py"
[ -f "${_swigpy}" ] || { echo "pymupdf-cp313: ${_swigpy} not found — MuPDF layout changed?" >&2; exit 1; }
if grep -q 'PyString_FromString' "${_swigpy}"; then
    sed -i 's/PyString_FromString/PyUnicode_FromString/g' "${_swigpy}"
    echo "pymupdf-cp313: patched PyString_FromString -> PyUnicode_FromString in swig.py"
fi
...
if ! "${PY}" -c 'import pipcl, clang.cindex' >/dev/null 2>&1; then
    echo "pymupdf-cp313: installing pinned build backends (pipcl==13, libclang==18.1.1)"
    "${PY}" -m pip install --disable-pip-version-check --no-warn-script-location \
        --prefix "${BLD}" "pipcl==13" "libclang==18.1.1"
fi
"${PY}" -c 'import pipcl, clang.cindex; print("pymupdf-cp313: pipcl + clang.cindex OK")'

# ── 4. SWIG from the catalog (host_tool), pinned to a cvcpkg-prefix copy ─────
_swig="$(command -v swig 2>/dev/null || true)"
[ -n "${_swig}" ] || { echo "pymupdf-cp313: swig not on PATH — is the 'swig' host_tool in the closure?" >&2; exit 1; }
case "${_swig}" in
    "${BLD}"/*|"${DEPS}"/*) : ;;   # a cvcpkg-prefix swig — good
    *) echo "pymupdf-cp313: WARNING using swig at ${_swig} (outside ${BLD}/${DEPS})" >&2 ;;
esac
...
[ -n "${SWIG_LIB:-}" ] && [ -f "${SWIG_LIB}/swig.swg" ] || {
    echo "pymupdf-cp313: could not locate swig.swg under ${BLD}/share/swig or ${DEPS}/share/swig" >&2
    echo "  (SWIG_LIB=${SWIG_LIB:-<unset>}) — swig would fail with 'Unknown directive'." >&2
    exit 1
}
echo "pymupdf-cp313: swig $("${_swig}" -version 2>/dev/null | awk '/SWIG Version/{print $3}') [${_swig}], SWIG_LIB=${SWIG_LIB}"

# ── 5. Build the wheel ──────────────────────────────────────────────────────
# PYMUPDF_SETUP_MUPDF_BUILD=<dir> -> setup.py's get_mupdf() uses this local tree
# and does NOT download.  PY_LIMITED_API=0 builds a version-specific cp313 wheel
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "pymupdf-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pymupdf-cp313: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || { echo "pymupdf-cp313: staged pymupdf/ not found under ${CVC_INSTALL_DIR}" >&2; ls -la "${CVC_INSTALL_DIR}" >&2 || true; exit 1; }
PKG_DIR="${SITE_PACKAGES}/pymupdf"
echo "pymupdf-cp313: staged into ${SITE_PACKAGES}"
...
print(f"pymupdf-cp313: rasterised {os.path.basename(os.environ['CVC_TESTPDF'])} "
      f"page0 -> {pix.width}x{pix.height} n={pix.n}, PNG {len(png)} bytes; bundled {libs}")
PYCHECK

echo "pymupdf-cp313: build + verification complete"
