#!/usr/bin/env bash
# recipes/cppy-cp312/build.sh — build cppy 1.3.1 FROM SOURCE (generated).
#
...
echo "cppy-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cppy-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cppy-cp312: built $(basename "${WHEEL}")"
