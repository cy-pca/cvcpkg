#!/usr/bin/env bash
# recipes/packaging-cp313/build.sh — build packaging 24.2 FROM SOURCE (generated).
#
...
echo "packaging-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "packaging-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "packaging-cp313: built $(basename "${WHEEL}")"
