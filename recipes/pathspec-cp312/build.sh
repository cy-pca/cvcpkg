#!/usr/bin/env bash
# recipes/pathspec-cp312/build.sh — build pathspec 1.1.1 FROM SOURCE (generated).
#
...
echo "pathspec-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pathspec-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pathspec-cp312: built $(basename "${WHEEL}")"
