#!/usr/bin/env bash
# recipes/certifi-cp313/build.sh — build certifi 2026.5.20 FROM SOURCE (generated).
#
...
echo "certifi-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "certifi-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "certifi-cp313: built $(basename "${WHEEL}")"
