#!/usr/bin/env bash
# recipes/httpcore-cp313t/build.sh — build httpcore 1.0.9 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp313t, hatch-fancy-pypi-readme-cp313t, packaging-cp313t, pathspec-cp313t, pluggy-cp313t, trove-classifiers-cp313t, calver-cp313t
...
echo "httpcore-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpcore-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpcore-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import httpcore"
