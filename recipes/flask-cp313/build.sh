#!/usr/bin/env bash
# recipes/flask-cp313/build.sh — build flask 3.1.3 FROM SOURCE (generated).
#
...
echo "flask-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "flask-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "flask-cp313: built $(basename "${WHEEL}")"
