#!/usr/bin/env bash
# recipes/flit-core-cp313t/build.sh — build flit-core 3.12.0 FROM SOURCE (generated).
#
...
echo "flit-core-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "flit-core-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "flit-core-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import flit_core"
