#!/usr/bin/env bash
# recipes/click-cp311/build.sh — build click 8.4.1 FROM SOURCE (generated).
#
...
echo "click-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "click-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "click-cp311: built $(basename "${WHEEL}")"
