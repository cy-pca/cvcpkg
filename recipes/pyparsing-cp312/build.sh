#!/usr/bin/env bash
# recipes/pyparsing-cp312/build.sh — build pyparsing 3.2.1 FROM SOURCE (generated).
#
...
echo "pyparsing-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyparsing-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyparsing-cp312: built $(basename "${WHEEL}")"
