#!/usr/bin/env bash
# recipes/six-cp312/build.sh — build six 1.17.0 FROM SOURCE (generated).
#
...
echo "six-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "six-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "six-cp312: built $(basename "${WHEEL}")"
