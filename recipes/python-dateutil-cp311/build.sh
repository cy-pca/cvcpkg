#!/usr/bin/env bash
# recipes/python-dateutil-cp311/build.sh — build python-dateutil 2.9.0.post0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311, wheel-cp311, setuptools-scm-cp311
...
echo "python-dateutil-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "python-dateutil-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "python-dateutil-cp311: built $(basename "${WHEEL}")"
