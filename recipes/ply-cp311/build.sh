#!/usr/bin/env bash
# recipes/ply-cp311/build.sh — build ply 3.11 FROM SOURCE (generated).
#
...
echo "ply-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "ply-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "ply-cp311: built $(basename "${WHEEL}")"
