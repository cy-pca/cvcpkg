#!/usr/bin/env bash
# recipes/uvicorn-cp313t/build.sh — build uvicorn 0.47.0 FROM SOURCE (generated).
#
...
echo "uvicorn-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "uvicorn-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "uvicorn-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import uvicorn"
