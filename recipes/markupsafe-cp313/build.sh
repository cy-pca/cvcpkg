#!/usr/bin/env bash
# recipes/markupsafe-cp313/build.sh — build MarkupSafe 3.0.3 FROM SOURCE for the cp313
...
echo "markupsafe-cp313: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools; print("markupsafe-cp313: setuptools", setuptools.__version__, setuptools.__file__)'
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "markupsafe-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "markupsafe-cp313: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "markupsafe-cp313: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "markupsafe-cp313: staged into ${SITE_PACKAGES}"
...
echo "markupsafe-cp313: build + verification complete"
