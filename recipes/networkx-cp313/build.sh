#!/usr/bin/env bash
# recipes/networkx-cp313/build.sh — build networkx 3.4.2 FROM SOURCE (generated).
#
...
echo "networkx-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "networkx-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "networkx-cp313: built $(basename "${WHEEL}")"
