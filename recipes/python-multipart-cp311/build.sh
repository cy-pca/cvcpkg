#!/usr/bin/env bash
# recipes/python-multipart-cp311/build.sh — build python-multipart 0.0.29 FROM SOURCE (generated).
#
...
echo "python-multipart-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "python-multipart-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "python-multipart-cp311: built $(basename "${WHEEL}")"
