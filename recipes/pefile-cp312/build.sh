#!/usr/bin/env bash
# recipes/pefile-cp312/build.sh — build pefile 2024.8.26 FROM SOURCE (generated).
#
...
echo "pefile-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pefile-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pefile-cp312: built $(basename "${WHEEL}")"
