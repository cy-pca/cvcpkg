#!/usr/bin/env bash
# recipes/imageio-cp312/build.sh — build imageio 2.36.1 FROM SOURCE (generated).
#
...
echo "imageio-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "imageio-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "imageio-cp312: built $(basename "${WHEEL}")"
