#!/usr/bin/env bash
# recipes/altgraph-cp312/build.sh — build altgraph 0.17.5 FROM SOURCE (generated).
#
...
echo "altgraph-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "altgraph-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "altgraph-cp312: built $(basename "${WHEEL}")"
