#!/usr/bin/env bash
# recipes/werkzeug-cp311/build.sh — build werkzeug 3.1.8 FROM SOURCE (generated).
#
...
echo "werkzeug-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "werkzeug-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "werkzeug-cp311: built $(basename "${WHEEL}")"
