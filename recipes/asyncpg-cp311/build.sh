#!/usr/bin/env bash
# recipes/asyncpg-cp311/build.sh — build asyncpg 0.31.0 FROM SOURCE (generated).
#
...
echo "asyncpg-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "asyncpg-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "asyncpg-cp311: built $(basename "${WHEEL}")"
