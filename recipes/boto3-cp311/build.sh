#!/usr/bin/env bash
# recipes/boto3-cp311/build.sh — build boto3 1.43.49 FROM SOURCE (generated).
#
...
echo "boto3-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "boto3-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "boto3-cp311: built $(basename "${WHEEL}")"
