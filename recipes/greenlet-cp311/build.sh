#!/usr/bin/env bash
# recipes/greenlet-cp311/build.sh — build greenlet 3.5.3 FROM SOURCE for the cp311
...
echo "greenlet-cp311: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools; print("greenlet-cp311: setuptools", setuptools.__version__, setuptools.__file__)'
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "greenlet-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "greenlet-cp311: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "greenlet-cp311: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "greenlet-cp311: staged into ${SITE_PACKAGES}"
...
echo "greenlet-cp311: build + verification complete"
