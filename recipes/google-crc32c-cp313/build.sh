#!/usr/bin/env bash
# recipes/google-crc32c-cp313/build.sh — build google-crc32c 1.8.0 FROM SOURCE (generated).
#
...
echo "google-crc32c-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-crc32c-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-crc32c-cp313: built $(basename "${WHEEL}")"
