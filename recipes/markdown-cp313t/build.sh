#!/usr/bin/env bash
# recipes/markdown-cp313t/build.sh — build markdown 3.10.3 FROM SOURCE (generated).
#
...
echo "markdown-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "markdown-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "markdown-cp313t: built $(basename "${WHEEL}")"
