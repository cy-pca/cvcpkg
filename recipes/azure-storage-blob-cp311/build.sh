#!/usr/bin/env bash
# recipes/azure-storage-blob-cp311/build.sh — build azure-storage-blob 12.30.0 FROM SOURCE (generated).
#
...
echo "azure-storage-blob-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "azure-storage-blob-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "azure-storage-blob-cp311: built $(basename "${WHEEL}")"
