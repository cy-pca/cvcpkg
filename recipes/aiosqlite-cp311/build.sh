#!/usr/bin/env bash
# recipes/aiosqlite-cp311/build.sh — build aiosqlite 0.22.1 FROM SOURCE (generated).
#
...
echo "aiosqlite-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "aiosqlite-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "aiosqlite-cp311: built $(basename "${WHEEL}")"
