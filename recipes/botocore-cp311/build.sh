#!/usr/bin/env bash
# recipes/botocore-cp311/build.sh — build botocore 1.43.49 FROM SOURCE (generated).
#
...
echo "botocore-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "botocore-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "botocore-cp311: built $(basename "${WHEEL}")"
