#!/usr/bin/env bash
# recipes/idna-cp312/build.sh — build idna 3.16 FROM SOURCE (generated).
#
...
echo "idna-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "idna-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "idna-cp312: built $(basename "${WHEEL}")"
