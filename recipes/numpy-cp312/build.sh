#!/usr/bin/env bash
# recipes/numpy-cp312/build.sh — build NumPy 2.4.6 FROM SOURCE (meson-python)
# against cvcpkg's OpenBLAS (LP64), then install the resulting wheel into the
# python312 interpreter's site-packages.
#
...
case "${CVC_PLATFORM}" in
  *)
    ...
    [ -n "${PKG_CONFIG:-}" ] || { echo "numpy-cp312: no pkg-config found in ${BLD}/bin, ${DEPS}/bin or PATH" >&2; exit 1; }
    ...
    if ! "${PKG_CONFIG}" --exists openblas; then
        echo "numpy-cp312: openblas.pc not found by ${PKG_CONFIG}" >&2
        echo "  PKG_CONFIG_LIBDIR=${PKG_CONFIG_LIBDIR}" >&2
        ls -la "${DEPS}/lib/pkgconfig" "${BLD}/lib/pkgconfig" 2>&1 | head -40 >&2
        exit 1
    fi
    echo "numpy-cp312: openblas $("${PKG_CONFIG}" --modversion openblas) via ${PKG_CONFIG}"
    BLAS_ARGS+=(
...
[ -n "${WHEEL}" ] || { echo "numpy-cp312: no wheel produced" >&2; exit 1; }
echo "numpy-cp312: built $(basename "${WHEEL}")"
...
[ -n "${NP_DIR}" ] || { echo "numpy-cp312: staged numpy/ not found" >&2; exit 1; }
...
if [ "${CVC_PLATFORM}" != "macos" ]; then
  command -v patchelf >/dev/null 2>&1 || { echo "numpy-cp312: patchelf missing" >&2; exit 1; }
...
np.show_config()
print("numpy-cp312 build + verification complete")
PYCHECK
