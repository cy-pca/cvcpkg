#!/usr/bin/env bash
# recipes/pyasn1-cp313/build.sh — build pyasn1 0.6.4 FROM SOURCE (generated).
#
...
echo "pyasn1-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyasn1-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyasn1-cp313: built $(basename "${WHEEL}")"
