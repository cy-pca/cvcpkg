#!/usr/bin/env bash
# recipes/s3transfer-cp312/build.sh — build s3transfer 0.19.1 FROM SOURCE (generated).
#
...
echo "s3transfer-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "s3transfer-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "s3transfer-cp312: built $(basename "${WHEEL}")"
