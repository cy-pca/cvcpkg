#!/usr/bin/env bash
# recipes/wheel-cp312/build.sh — build wheel 0.45.1 FROM SOURCE (generated).
#
...
echo "wheel-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "wheel-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "wheel-cp312: built $(basename "${WHEEL}")"
