#!/usr/bin/env bash
# recipes/tqdm-cp313/build.sh — build tqdm 4.67.3 FROM SOURCE (generated).
#
...
echo "tqdm-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tqdm-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tqdm-cp313: built $(basename "${WHEEL}")"
