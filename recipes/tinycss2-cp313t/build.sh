#!/usr/bin/env bash
# recipes/tinycss2-cp313t/build.sh — build tinycss2 1.5.1 FROM SOURCE (generated).
#
...
echo "tinycss2-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tinycss2-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tinycss2-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import tinycss2"
