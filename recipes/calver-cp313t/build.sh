#!/usr/bin/env bash
# recipes/calver-cp313t/build.sh — build calver 2022.6.26 FROM SOURCE (generated).
#
...
echo "calver-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "calver-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "calver-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import calver"
