#!/usr/bin/env bash
# recipes/typing-extensions-cp313/build.sh — build typing-extensions 4.15.0 FROM SOURCE (generated).
#
...
echo "typing-extensions-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "typing-extensions-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "typing-extensions-cp313: built $(basename "${WHEEL}")"
