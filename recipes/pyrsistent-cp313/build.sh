#!/usr/bin/env bash
# recipes/pyrsistent-cp313/build.sh — build pyrsistent 0.20.0 FROM SOURCE (generated).
#
...
echo "pyrsistent-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyrsistent-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyrsistent-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pyrsistent"
