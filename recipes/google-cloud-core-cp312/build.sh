#!/usr/bin/env bash
# recipes/google-cloud-core-cp312/build.sh — build google-cloud-core 2.6.0 FROM SOURCE (generated).
#
...
echo "google-cloud-core-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-cloud-core-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-cloud-core-cp312: built $(basename "${WHEEL}")"
