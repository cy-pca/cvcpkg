#!/usr/bin/env bash
# recipes/sqlalchemy-cp311/build.sh — build sqlalchemy 2.0.50 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311, cython-cp311
...
echo "sqlalchemy-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "sqlalchemy-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "sqlalchemy-cp311: built $(basename "${WHEEL}")"
