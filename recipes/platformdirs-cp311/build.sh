#!/usr/bin/env bash
# recipes/platformdirs-cp311/build.sh — build platformdirs 4.11.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatch-vcs-cp311, hatchling-cp311, packaging-cp311, pathspec-cp311, pluggy-cp311, trove-classifiers-cp311, calver-cp311
...
echo "platformdirs-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "platformdirs-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "platformdirs-cp311: built $(basename "${WHEEL}")"
