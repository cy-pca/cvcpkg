#!/usr/bin/env bash
# recipes/poetry-core-cp313/build.sh — build poetry-core 2.4.1 FROM SOURCE (generated).
#
...
echo "poetry-core-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "poetry-core-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "poetry-core-cp313: built $(basename "${WHEEL}")"
