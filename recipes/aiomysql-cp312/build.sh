#!/usr/bin/env bash
# recipes/aiomysql-cp312/build.sh — build aiomysql 0.3.2 FROM SOURCE (generated).
#
...
echo "aiomysql-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "aiomysql-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "aiomysql-cp312: built $(basename "${WHEEL}")"
