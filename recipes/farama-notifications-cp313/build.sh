#!/usr/bin/env bash
# recipes/farama-notifications-cp313/build.sh — build farama-notifications 0.0.6 FROM SOURCE (generated).
#
...
echo "farama-notifications-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "farama-notifications-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "farama-notifications-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import farama_notifications"
