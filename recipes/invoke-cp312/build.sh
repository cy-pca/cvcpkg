#!/usr/bin/env bash
# recipes/invoke-cp312/build.sh — build invoke 3.0.3 FROM SOURCE (generated).
#
...
echo "invoke-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "invoke-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "invoke-cp312: built $(basename "${WHEEL}")"
