#!/usr/bin/env bash
# recipes/google-auth-cp311/build.sh — build google-auth 2.56.0 FROM SOURCE (generated).
#
...
echo "google-auth-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-auth-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-auth-cp311: built $(basename "${WHEEL}")"
