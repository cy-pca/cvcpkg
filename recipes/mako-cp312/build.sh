#!/usr/bin/env bash
# recipes/mako-cp312/build.sh — build mako 1.3.12 FROM SOURCE (generated).
#
...
echo "mako-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mako-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mako-cp312: built $(basename "${WHEEL}")"
