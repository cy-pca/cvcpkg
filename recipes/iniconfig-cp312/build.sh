#!/usr/bin/env bash
# recipes/iniconfig-cp312/build.sh — build iniconfig 2.3.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, setuptools-scm-cp312
...
echo "iniconfig-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "iniconfig-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "iniconfig-cp312: built $(basename "${WHEEL}")"
