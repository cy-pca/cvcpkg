#!/usr/bin/env bash
# recipes/blinker-cp311/build.sh — build blinker 1.9.0 FROM SOURCE (generated).
#
...
echo "blinker-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "blinker-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "blinker-cp311: built $(basename "${WHEEL}")"
