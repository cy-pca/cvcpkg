#!/usr/bin/env bash
# recipes/kiwisolver-cp312/build.sh — build kiwisolver 1.4.8 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, setuptools-scm-cp312, cppy-cp312
...
echo "kiwisolver-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "kiwisolver-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "kiwisolver-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import kiwisolver"
