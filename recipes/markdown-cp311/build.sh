#!/usr/bin/env bash
# recipes/markdown-cp311/build.sh — build markdown 3.10.3 FROM SOURCE (generated).
#
...
echo "markdown-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "markdown-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "markdown-cp311: built $(basename "${WHEEL}")"
