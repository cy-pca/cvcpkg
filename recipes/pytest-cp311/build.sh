#!/usr/bin/env bash
# recipes/pytest-cp311/build.sh — build pytest 9.1.1 FROM SOURCE (generated).
#
...
echo "pytest-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pytest-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pytest-cp311: built $(basename "${WHEEL}")"
