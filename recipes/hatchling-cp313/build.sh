#!/usr/bin/env bash
# recipes/hatchling-cp313/build.sh — build hatchling 1.31.0 FROM SOURCE (generated).
#
...
echo "hatchling-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatchling-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatchling-cp313: built $(basename "${WHEEL}")"
