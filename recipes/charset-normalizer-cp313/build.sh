#!/usr/bin/env bash
# recipes/charset-normalizer-cp313/build.sh — build charset-normalizer 3.4.9 FROM SOURCE (generated).
#
...
echo "charset-normalizer-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "charset-normalizer-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "charset-normalizer-cp313: built $(basename "${WHEEL}")"
