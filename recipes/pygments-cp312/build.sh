#!/usr/bin/env bash
# recipes/pygments-cp312/build.sh — build pygments 2.20.0 FROM SOURCE (generated).
#
...
echo "pygments-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pygments-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pygments-cp312: built $(basename "${WHEEL}")"
