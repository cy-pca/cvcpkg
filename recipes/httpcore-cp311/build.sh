#!/usr/bin/env bash
# recipes/httpcore-cp311/build.sh — build httpcore 1.0.9 FROM SOURCE (generated).
#
...
echo "httpcore-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpcore-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpcore-cp311: built $(basename "${WHEEL}")"
