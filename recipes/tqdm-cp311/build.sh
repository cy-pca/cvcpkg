#!/usr/bin/env bash
# recipes/tqdm-cp311/build.sh — build tqdm 4.67.3 FROM SOURCE (generated).
#
...
echo "tqdm-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tqdm-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tqdm-cp311: built $(basename "${WHEEL}")"
