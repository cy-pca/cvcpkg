#!/usr/bin/env bash
# recipes/anyio-cp311/build.sh — build anyio 4.13.0 FROM SOURCE (generated).
#
...
echo "anyio-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "anyio-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "anyio-cp311: built $(basename "${WHEEL}")"
