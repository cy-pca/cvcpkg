#!/usr/bin/env bash
# recipes/imageio-cp311/build.sh — build imageio 2.36.1 FROM SOURCE (generated).
#
...
echo "imageio-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "imageio-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "imageio-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import imageio"
