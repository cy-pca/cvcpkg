#!/usr/bin/env bash
# recipes/hatchling-cp313t/build.sh — build hatchling 1.31.0 FROM SOURCE (generated).
#
...
echo "hatchling-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatchling-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatchling-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import hatchling"
