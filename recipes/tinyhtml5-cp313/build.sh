#!/usr/bin/env bash
# recipes/tinyhtml5-cp313/build.sh — build tinyhtml5 2.1.0 FROM SOURCE (generated).
#
...
echo "tinyhtml5-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tinyhtml5-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tinyhtml5-cp313: built $(basename "${WHEEL}")"
