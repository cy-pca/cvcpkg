#!/usr/bin/env bash
# recipes/platformdirs-cp313t/build.sh — build platformdirs 4.11.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatch-vcs-cp313t, hatchling-cp313t, packaging-cp313t, pathspec-cp313t, pluggy-cp313t, trove-classifiers-cp313t, calver-cp313t
...
echo "platformdirs-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "platformdirs-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "platformdirs-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import platformdirs"
