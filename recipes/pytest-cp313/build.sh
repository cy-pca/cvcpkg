#!/usr/bin/env bash
# recipes/pytest-cp313/build.sh — build pytest 9.1.1 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, setuptools-scm-cp313, trove-classifiers-cp313
...
echo "pytest-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pytest-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pytest-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pytest"
