#!/usr/bin/env bash
# recipes/iniconfig-cp311/build.sh — build iniconfig 2.3.0 FROM SOURCE (generated).
#
...
echo "iniconfig-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "iniconfig-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "iniconfig-cp311: built $(basename "${WHEEL}")"
