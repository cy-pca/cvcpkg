#!/usr/bin/env bash
# recipes/attrs-cp313/build.sh — build attrs 26.1.0 FROM SOURCE (generated).
#
...
echo "attrs-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "attrs-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "attrs-cp313: built $(basename "${WHEEL}")"
