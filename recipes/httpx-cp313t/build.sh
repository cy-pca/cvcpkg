#!/usr/bin/env bash
# recipes/httpx-cp313t/build.sh — build httpx 0.28.1 FROM SOURCE (generated).
#
...
echo "httpx-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpx-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpx-cp313t: built $(basename "${WHEEL}")"
