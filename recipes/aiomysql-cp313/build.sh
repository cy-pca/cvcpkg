#!/usr/bin/env bash
# recipes/aiomysql-cp313/build.sh — build aiomysql 0.3.2 FROM SOURCE (generated).
#
...
echo "aiomysql-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "aiomysql-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "aiomysql-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import aiomysql"
