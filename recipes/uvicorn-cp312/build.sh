#!/usr/bin/env bash
# recipes/uvicorn-cp312/build.sh — build uvicorn 0.47.0 FROM SOURCE (generated).
#
...
echo "uvicorn-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "uvicorn-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "uvicorn-cp312: built $(basename "${WHEEL}")"
