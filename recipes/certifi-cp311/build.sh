#!/usr/bin/env bash
# recipes/certifi-cp311/build.sh — build certifi 2026.5.20 FROM SOURCE (generated).
#
...
echo "certifi-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "certifi-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "certifi-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import certifi"
