#!/usr/bin/env bash
# recipes/setuptools-scm-cp311/build.sh — build setuptools_scm 8.3.1 FROM SOURCE (sdist) for the
# cp311 interpreter column of the per-interpreter wheel matrix.
#
...
echo "setuptools-scm-cp311: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, packaging; print("setuptools-scm-cp311: setuptools", setuptools.__version__, "packaging", packaging.__version__)'
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "setuptools-scm-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "setuptools-scm-cp311: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "setuptools-scm-cp311: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "setuptools-scm-cp311: staged into ${SITE_PACKAGES}"
...
echo "setuptools-scm-cp311: build + verification complete"
