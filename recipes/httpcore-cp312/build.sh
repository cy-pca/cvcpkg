#!/usr/bin/env bash
# recipes/httpcore-cp312/build.sh — build httpcore 1.0.9 FROM SOURCE (generated).
#
...
echo "httpcore-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpcore-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpcore-cp312: built $(basename "${WHEEL}")"
