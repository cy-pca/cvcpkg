#!/usr/bin/env bash
# recipes/pefile-cp313/build.sh — build pefile 2024.8.26 FROM SOURCE (generated).
#
...
echo "pefile-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pefile-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pefile-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pefile"
