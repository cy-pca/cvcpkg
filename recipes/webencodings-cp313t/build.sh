#!/usr/bin/env bash
# recipes/webencodings-cp313t/build.sh — build webencodings 0.6.1 FROM SOURCE (generated).
#
...
echo "webencodings-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "webencodings-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "webencodings-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import webencodings"
