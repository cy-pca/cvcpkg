#!/usr/bin/env bash
# recipes/beniget-cp311/build.sh — build beniget 0.5.0 FROM SOURCE (generated).
#
...
echo "beniget-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "beniget-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "beniget-cp311: built $(basename "${WHEEL}")"
