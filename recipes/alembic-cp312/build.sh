#!/usr/bin/env bash
# recipes/alembic-cp312/build.sh — build alembic 1.18.4 FROM SOURCE (generated).
#
...
echo "alembic-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "alembic-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "alembic-cp312: built $(basename "${WHEEL}")"
