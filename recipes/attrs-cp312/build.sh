#!/usr/bin/env bash
# recipes/attrs-cp312/build.sh — build attrs 26.1.0 FROM SOURCE (generated).
#
...
echo "attrs-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "attrs-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "attrs-cp312: built $(basename "${WHEEL}")"
