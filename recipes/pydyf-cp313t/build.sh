#!/usr/bin/env bash
# recipes/pydyf-cp313t/build.sh — build pydyf 0.12.1 FROM SOURCE (generated).
#
...
echo "pydyf-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pydyf-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydyf-cp313t: built $(basename "${WHEEL}")"
