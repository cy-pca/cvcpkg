#!/usr/bin/env bash
# recipes/pyproject-metadata-cp313/build.sh — build pyproject-metadata 0.9.1 FROM SOURCE (generated).
#
...
echo "pyproject-metadata-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyproject-metadata-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyproject-metadata-cp313: built $(basename "${WHEEL}")"
