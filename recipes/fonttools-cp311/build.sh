#!/usr/bin/env bash
# recipes/fonttools-cp311/build.sh — build fonttools 4.63.0 FROM SOURCE (generated).
#
...
echo "fonttools-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fonttools-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fonttools-cp311: built $(basename "${WHEEL}")"
