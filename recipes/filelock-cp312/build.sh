#!/usr/bin/env bash
# recipes/filelock-cp312/build.sh — build filelock 3.16.1 FROM SOURCE (generated).
#
...
echo "filelock-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "filelock-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "filelock-cp312: built $(basename "${WHEEL}")"
