#!/usr/bin/env bash
# recipes/blinker-cp313/build.sh — build blinker 1.9.0 FROM SOURCE (generated).
#
...
echo "blinker-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "blinker-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "blinker-cp313: built $(basename "${WHEEL}")"
