#!/usr/bin/env bash
# recipes/annotated-types-cp313t/build.sh — build annotated-types 0.7.0 FROM SOURCE (generated).
#
...
echo "annotated-types-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "annotated-types-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "annotated-types-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import annotated_types"
