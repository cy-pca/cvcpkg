#!/usr/bin/env bash
# recipes/gast-cp313/build.sh — build gast 0.7.0 FROM SOURCE (generated).
#
...
echo "gast-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gast-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gast-cp313: built $(basename "${WHEEL}")"
