#!/usr/bin/env bash
# recipes/cycler-cp311/build.sh — build cycler 0.12.1 FROM SOURCE (generated).
#
...
echo "cycler-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cycler-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cycler-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import cycler"
