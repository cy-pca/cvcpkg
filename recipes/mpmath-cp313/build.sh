#!/usr/bin/env bash
# recipes/mpmath-cp313/build.sh — build mpmath 1.3.0 FROM SOURCE (generated).
#
...
echo "mpmath-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mpmath-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mpmath-cp313: built $(basename "${WHEEL}")"
