#!/usr/bin/env bash
# recipes/beniget-cp313/build.sh — build beniget 0.5.0 FROM SOURCE (generated).
#
...
echo "beniget-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "beniget-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "beniget-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import beniget"
