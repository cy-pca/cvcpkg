#!/usr/bin/env bash
# recipes/pythran-cp312/build.sh — build pythran 0.18.1 FROM SOURCE (generated).
#
...
echo "pythran-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pythran-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pythran-cp312: built $(basename "${WHEEL}")"
