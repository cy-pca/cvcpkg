#!/usr/bin/env bash
# recipes/meson-python-cp311/build.sh — build meson-python 0.18.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: meson, packaging-cp311, pyproject-metadata-cp311
...
echo "meson-python-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "meson-python-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "meson-python-cp311: built $(basename "${WHEEL}")"
