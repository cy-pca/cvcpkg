#!/usr/bin/env bash
# recipes/setuptools-cp312/build.sh — build setuptools 80.9.0 FROM SOURCE (generated).
#
...
echo "setuptools-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "setuptools-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "setuptools-cp312: built $(basename "${WHEEL}")"
