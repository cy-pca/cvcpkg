#!/usr/bin/env bash
# recipes/h11-cp312/build.sh — build h11 0.16.0 FROM SOURCE (generated).
#
...
echo "h11-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "h11-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "h11-cp312: built $(basename "${WHEEL}")"
