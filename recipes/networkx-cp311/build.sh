#!/usr/bin/env bash
# recipes/networkx-cp311/build.sh — build networkx 3.4.2 FROM SOURCE (generated).
#
...
echo "networkx-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "networkx-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "networkx-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import networkx"
