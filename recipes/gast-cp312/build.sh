#!/usr/bin/env bash
# recipes/gast-cp312/build.sh — build gast 0.7.0 FROM SOURCE (generated).
#
...
echo "gast-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gast-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gast-cp312: built $(basename "${WHEEL}")"
