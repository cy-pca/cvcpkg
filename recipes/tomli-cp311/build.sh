#!/usr/bin/env bash
# recipes/tomli-cp311/build.sh — build tomli 2.4.1 FROM SOURCE (generated).
#
...
echo "tomli-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tomli-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tomli-cp311: built $(basename "${WHEEL}")"
