#!/usr/bin/env bash
# recipes/pycparser-cp311/build.sh — build pycparser 3.0 FROM SOURCE (generated).
#
...
echo "pycparser-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pycparser-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pycparser-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pycparser"
