#!/usr/bin/env bash
# recipes/starlette-cp312/build.sh — build starlette 1.0.1 FROM SOURCE (generated).
#
...
echo "starlette-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "starlette-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "starlette-cp312: built $(basename "${WHEEL}")"
