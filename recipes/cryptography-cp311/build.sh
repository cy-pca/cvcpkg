#!/usr/bin/env bash
# recipes/cryptography-cp311/build.sh — build cryptography 48.0.0 FROM SOURCE for
# the cp311 interpreter column of the per-interpreter wheel matrix.
#
...
echo "cryptography-cp311: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, cffi, maturin; print("cryptography-cp311: setuptools", setuptools.__version__, "cffi", cffi.__version__, "+ maturin backend")'
...
[ -n "${MATURIN_EXE}" ] || {
    echo "cryptography-cp311: no 'maturin' executable on PATH — the maturin-cp311" >&2
    echo "  build dep supplies it in ${BLD}/bin; is it in the closure?" >&2
    exit 1
}
echo "cryptography-cp311: maturin $("${MATURIN_EXE}" --version) [${MATURIN_EXE}]"
...
_require_prefix_tool() {
    ...
    [ -n "${resolved}" ] || {
        echo "cryptography-cp311: ${tool} not on PATH — is the 'rust' build dep in the closure?" >&2
        exit 1
    }
    case "${resolved}" in
        "${BLD}"/*|"${DEPS}"/*) echo "${resolved}" ;;
        *)
            echo "cryptography-cp311: refusing ${tool} at ${resolved} — it is outside the" >&2
...
echo "cryptography-cp311: cargo $("${CARGO_BIN}" --version) [${CARGO_BIN}]"
echo "cryptography-cp311: rustc $("${RUSTC_BIN}" --version) [${RUSTC_BIN}]"
...
if [ -n "${CVC_CARGO_OFFLINE:-}" ]; then
    ...
    echo "cryptography-cp311: CARGO_NET_OFFLINE=true — ${CARGO_HOME} must already"
    echo "  contain every crate in Cargo.lock, or cargo will fail rather than fetch."
else
    export CARGO_NET_OFFLINE=false
    echo "cryptography-cp311: NOTE — cargo will FETCH crates.io. This sdist ships"
...
[ -e "${OPENSSL_DIR}/include/openssl/ssl.h" ] || {
    echo "cryptography-cp311: ${OPENSSL_DIR}/include/openssl/ssl.h missing — is the" >&2
...
echo "cryptography-cp311: OPENSSL_DIR=${OPENSSL_DIR} (version ${CVC_OPENSSL_VERSION:-unknown})"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "cryptography-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cryptography-cp311: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "cryptography-cp311: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "cryptography-cp311: staged into ${SITE_PACKAGES}"
...
if [ "${CVC_PLATFORM}" = "macos" ]; then
    ...
    command -v install_name_tool >/dev/null 2>&1 || {
        echo "cryptography-cp311: install_name_tool required on macOS but not found" >&2; exit 1; }
    ...
    command -v patchelf >/dev/null 2>&1 || {
        echo "cryptography-cp311: patchelf required on ${CVC_PLATFORM} but not on PATH" >&2; exit 1; }
    while IFS= read -r -d '' _so; do
        _rel="$(_relpath_to_lib "${_so}")"
...
echo "cryptography-cp311: build + verification complete"
