#!/usr/bin/env bash
# recipes/blinker-cp313t/build.sh — build blinker 1.9.0 FROM SOURCE (generated).
#
...
echo "blinker-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "blinker-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "blinker-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import blinker"
