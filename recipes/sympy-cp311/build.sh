#!/usr/bin/env bash
# recipes/sympy-cp311/build.sh — build sympy 1.13.3 FROM SOURCE (generated).
#
...
echo "sympy-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "sympy-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "sympy-cp311: built $(basename "${WHEEL}")"
