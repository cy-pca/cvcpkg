#!/usr/bin/env bash
# recipes/pyjwt-cp311/build.sh — build pyjwt 2.13.0 FROM SOURCE (generated).
#
...
echo "pyjwt-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyjwt-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyjwt-cp311: built $(basename "${WHEEL}")"
