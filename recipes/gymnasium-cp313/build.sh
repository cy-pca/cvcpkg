#!/usr/bin/env bash
# recipes/gymnasium-cp313/build.sh — build gymnasium 1.3.0 FROM SOURCE (generated).
#
...
echo "gymnasium-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gymnasium-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gymnasium-cp313: built $(basename "${WHEEL}")"
