#!/usr/bin/env bash
# recipes/msal-cp313/build.sh — build msal 1.37.0 FROM SOURCE (generated).
#
...
echo "msal-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "msal-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "msal-cp313: built $(basename "${WHEEL}")"
