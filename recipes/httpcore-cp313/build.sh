#!/usr/bin/env bash
# recipes/httpcore-cp313/build.sh — build httpcore 1.0.9 FROM SOURCE (generated).
#
...
echo "httpcore-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpcore-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpcore-cp313: built $(basename "${WHEEL}")"
