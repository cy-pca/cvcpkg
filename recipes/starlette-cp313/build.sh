#!/usr/bin/env bash
# recipes/starlette-cp313/build.sh — build starlette 1.0.1 FROM SOURCE (generated).
#
...
echo "starlette-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "starlette-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "starlette-cp313: built $(basename "${WHEEL}")"
