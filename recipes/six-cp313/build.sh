#!/usr/bin/env bash
# recipes/six-cp313/build.sh — build six 1.17.0 FROM SOURCE (generated).
#
...
echo "six-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "six-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "six-cp313: built $(basename "${WHEEL}")"
