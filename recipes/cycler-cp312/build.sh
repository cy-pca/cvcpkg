#!/usr/bin/env bash
# recipes/cycler-cp312/build.sh — build cycler 0.12.1 FROM SOURCE (generated).
#
...
echo "cycler-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cycler-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cycler-cp312: built $(basename "${WHEEL}")"
