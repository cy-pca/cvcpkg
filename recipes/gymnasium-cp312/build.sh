#!/usr/bin/env bash
# recipes/gymnasium-cp312/build.sh — build gymnasium 1.3.0 FROM SOURCE (generated).
#
...
echo "gymnasium-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gymnasium-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gymnasium-cp312: built $(basename "${WHEEL}")"
