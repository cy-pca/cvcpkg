#!/usr/bin/env bash
# recipes/jmespath-cp313/build.sh — build jmespath 1.1.0 FROM SOURCE (generated).
#
...
echo "jmespath-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jmespath-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jmespath-cp313: built $(basename "${WHEEL}")"
