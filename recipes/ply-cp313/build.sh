#!/usr/bin/env bash
# recipes/ply-cp313/build.sh — build ply 3.11 FROM SOURCE (generated).
#
...
echo "ply-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "ply-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "ply-cp313: built $(basename "${WHEEL}")"
