#!/usr/bin/env bash
# recipes/pyphen-cp311/build.sh — build pyphen 0.18.1 FROM SOURCE (generated).
#
...
echo "pyphen-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyphen-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyphen-cp311: built $(basename "${WHEEL}")"
