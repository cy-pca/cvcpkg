#!/usr/bin/env bash
# recipes/trimesh-cp311/build.sh — build trimesh 5.0.0 FROM SOURCE (generated).
#
...
echo "trimesh-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trimesh-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trimesh-cp311: built $(basename "${WHEEL}")"
