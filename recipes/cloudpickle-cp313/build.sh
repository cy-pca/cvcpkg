#!/usr/bin/env bash
# recipes/cloudpickle-cp313/build.sh — build cloudpickle 3.1.2 FROM SOURCE (generated).
#
...
echo "cloudpickle-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cloudpickle-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cloudpickle-cp313: built $(basename "${WHEEL}")"
