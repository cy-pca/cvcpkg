#!/usr/bin/env bash
# recipes/typing-extensions-cp311/build.sh — build typing-extensions 4.15.0 FROM SOURCE (generated).
#
...
echo "typing-extensions-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "typing-extensions-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "typing-extensions-cp311: built $(basename "${WHEEL}")"
