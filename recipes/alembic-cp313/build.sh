#!/usr/bin/env bash
# recipes/alembic-cp313/build.sh — build alembic 1.18.4 FROM SOURCE (generated).
#
...
echo "alembic-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "alembic-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "alembic-cp313: built $(basename "${WHEEL}")"
