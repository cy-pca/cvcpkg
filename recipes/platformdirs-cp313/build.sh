#!/usr/bin/env bash
# recipes/platformdirs-cp313/build.sh — build platformdirs 4.11.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatch-vcs-cp313, hatchling-cp313, packaging-cp313, pathspec-cp313, pluggy-cp313, trove-classifiers-cp313, calver-cp313
...
echo "platformdirs-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "platformdirs-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "platformdirs-cp313: built $(basename "${WHEEL}")"
