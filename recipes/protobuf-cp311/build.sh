#!/usr/bin/env bash
# recipes/protobuf-cp311/build.sh — build protobuf 7.35.1 FROM SOURCE (generated).
#
...
echo "protobuf-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "protobuf-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "protobuf-cp311: built $(basename "${WHEEL}")"
