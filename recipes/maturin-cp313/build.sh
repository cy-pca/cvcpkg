#!/usr/bin/env bash
# recipes/maturin-cp313/build.sh — build maturin 1.14.1 FROM SOURCE for the
# cp313 interpreter column.
#
...
echo "maturin-cp313: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, setuptools_rust, wheel; print("maturin-cp313: setuptools", setuptools.__version__, "+ setuptools_rust + wheel")'
...
_require_prefix_tool() {
    ...
    [ -n "${resolved}" ] || {
        echo "maturin-cp313: ${tool} not on PATH — is the 'rust' build dep in the closure?" >&2
        exit 1
    }
    case "${resolved}" in
        "${BLD}"/*|"${DEPS}"/*) echo "${resolved}" ;;
        *)
            echo "maturin-cp313: refusing ${tool} at ${resolved} — it is outside the cvcpkg" >&2
...
echo "maturin-cp313: cargo $("${CARGO_BIN}" --version) [${CARGO_BIN}]"
echo "maturin-cp313: rustc $("${RUSTC_BIN}" --version) [${RUSTC_BIN}]"
...
if [ -n "${CVC_CARGO_OFFLINE:-}" ]; then
    ...
    echo "maturin-cp313: CARGO_NET_OFFLINE=true — ${CARGO_HOME} must already contain"
    echo "  every crate in Cargo.lock, or cargo will fail rather than fetch."
else
    export CARGO_NET_OFFLINE=false
    echo "maturin-cp313: NOTE — cargo will FETCH crates.io. maturin's sdist ships"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "maturin-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "maturin-cp313: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "maturin-cp313: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "maturin-cp313: staged into ${SITE_PACKAGES}"
...
[ -n "${MATURIN_EXE}" ] || {
    echo "maturin-cp313: no maturin executable staged under ${CVC_INSTALL_DIR}" >&2
    ls -la "${CVC_INSTALL_DIR}"/bin "${CVC_INSTALL_DIR}"/Scripts 2>&1 | head -40 >&2 || true
    exit 1
}
echo "maturin-cp313: staged executable ${MATURIN_EXE}"
"${MATURIN_EXE}" --version
...
echo "maturin-cp313: build + verification complete"
