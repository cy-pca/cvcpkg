#!/usr/bin/env bash
# recipes/pyrsistent-cp311/build.sh — build pyrsistent 0.20.0 FROM SOURCE (generated).
#
...
echo "pyrsistent-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyrsistent-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyrsistent-cp311: built $(basename "${WHEEL}")"
