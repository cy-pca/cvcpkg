#!/usr/bin/env bash
# recipes/pymysql-cp313/build.sh — build pymysql 1.2.0 FROM SOURCE (generated).
#
...
echo "pymysql-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pymysql-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pymysql-cp313: built $(basename "${WHEEL}")"
