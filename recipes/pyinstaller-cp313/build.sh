#!/usr/bin/env bash
# recipes/pyinstaller-cp313/build.sh — build PyInstaller 6.21.0 FROM SOURCE for the cp313
# interpreter column.
#
...
echo "pyinstaller-cp313: building against ${PY_EXE}"
...
export PYTHONPATH="${BLD}/lib/python3.13/site-packages${PYTHONPATH:+:${PYTHONPATH}}"
...
echo "pyinstaller-cp313: build + verification complete"
