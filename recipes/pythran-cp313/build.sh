#!/usr/bin/env bash
# recipes/pythran-cp313/build.sh — build pythran 0.18.1 FROM SOURCE (generated).
#
...
echo "pythran-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pythran-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pythran-cp313: built $(basename "${WHEEL}")"
