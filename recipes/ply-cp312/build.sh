#!/usr/bin/env bash
# recipes/ply-cp312/build.sh — build ply 3.11 FROM SOURCE (generated).
#
...
echo "ply-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "ply-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "ply-cp312: built $(basename "${WHEEL}")"
