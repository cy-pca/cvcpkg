#!/usr/bin/env bash
# recipes/google-resumable-media-cp313/build.sh — build google-resumable-media 2.10.0 FROM SOURCE (generated).
#
...
echo "google-resumable-media-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-resumable-media-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-resumable-media-cp313: built $(basename "${WHEEL}")"
