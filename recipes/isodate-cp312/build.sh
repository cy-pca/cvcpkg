#!/usr/bin/env bash
# recipes/isodate-cp312/build.sh — build isodate 0.7.2 FROM SOURCE (generated).
#
...
echo "isodate-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "isodate-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "isodate-cp312: built $(basename "${WHEEL}")"
