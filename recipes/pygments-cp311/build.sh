#!/usr/bin/env bash
# recipes/pygments-cp311/build.sh — build pygments 2.20.0 FROM SOURCE (generated).
#
...
echo "pygments-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pygments-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pygments-cp311: built $(basename "${WHEEL}")"
