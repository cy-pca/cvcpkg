#!/usr/bin/env bash
# recipes/paramiko-cp313/build.sh — build paramiko 5.0.0 FROM SOURCE (generated).
#
...
echo "paramiko-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "paramiko-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "paramiko-cp313: built $(basename "${WHEEL}")"
