#!/usr/bin/env bash
# recipes/invoke-cp311/build.sh — build invoke 3.0.3 FROM SOURCE (generated).
#
...
echo "invoke-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "invoke-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "invoke-cp311: built $(basename "${WHEEL}")"
