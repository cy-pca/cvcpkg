#!/usr/bin/env bash
# recipes/werkzeug-cp313t/build.sh — build werkzeug 3.1.8 FROM SOURCE (generated).
#
...
echo "werkzeug-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "werkzeug-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "werkzeug-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import werkzeug"
