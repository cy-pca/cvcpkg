#!/usr/bin/env bash
# recipes/werkzeug-cp313/build.sh — build werkzeug 3.1.8 FROM SOURCE (generated).
#
...
echo "werkzeug-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "werkzeug-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "werkzeug-cp313: built $(basename "${WHEEL}")"
