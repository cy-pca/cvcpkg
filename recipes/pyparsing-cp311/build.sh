#!/usr/bin/env bash
# recipes/pyparsing-cp311/build.sh — build pyparsing 3.2.1 FROM SOURCE (generated).
#
...
echo "pyparsing-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyparsing-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyparsing-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pyparsing"
