#!/usr/bin/env bash
# recipes/altgraph-cp313/build.sh — build altgraph 0.17.5 FROM SOURCE (generated).
#
...
echo "altgraph-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "altgraph-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "altgraph-cp313: built $(basename "${WHEEL}")"
