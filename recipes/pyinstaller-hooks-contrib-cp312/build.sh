#!/usr/bin/env bash
# recipes/pyinstaller-hooks-contrib-cp312/build.sh — build pyinstaller-hooks-contrib 2026.6 FROM SOURCE (generated).
#
...
echo "pyinstaller-hooks-contrib-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyinstaller-hooks-contrib-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyinstaller-hooks-contrib-cp312: built $(basename "${WHEEL}")"
