#!/usr/bin/env bash
# recipes/fsspec-cp312/build.sh — build fsspec 2024.10.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp312, hatch-vcs-cp312, packaging-cp312, pathspec-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "fsspec-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fsspec-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fsspec-cp312: built $(basename "${WHEEL}")"
