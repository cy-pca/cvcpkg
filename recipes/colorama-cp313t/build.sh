#!/usr/bin/env bash
# recipes/colorama-cp313t/build.sh — build colorama 0.4.6 FROM SOURCE (generated).
#
...
echo "colorama-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "colorama-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "colorama-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import colorama"
