#!/usr/bin/env bash
# recipes/starlette-cp311/build.sh — build starlette 1.0.1 FROM SOURCE (generated).
#
...
echo "starlette-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "starlette-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "starlette-cp311: built $(basename "${WHEEL}")"
