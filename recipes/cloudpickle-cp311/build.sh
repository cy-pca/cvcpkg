#!/usr/bin/env bash
# recipes/cloudpickle-cp311/build.sh — build cloudpickle 3.1.2 FROM SOURCE (generated).
#
...
echo "cloudpickle-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cloudpickle-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cloudpickle-cp311: built $(basename "${WHEEL}")"
