#!/usr/bin/env bash
# recipes/kiwisolver-cp311/build.sh — build kiwisolver 1.4.8 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311, setuptools-scm-cp311, cppy-cp311
...
echo "kiwisolver-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "kiwisolver-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "kiwisolver-cp311: built $(basename "${WHEEL}")"
