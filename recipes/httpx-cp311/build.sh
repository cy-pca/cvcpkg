#!/usr/bin/env bash
# recipes/httpx-cp311/build.sh — build httpx 0.28.1 FROM SOURCE (generated).
#
...
echo "httpx-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpx-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpx-cp311: built $(basename "${WHEEL}")"
