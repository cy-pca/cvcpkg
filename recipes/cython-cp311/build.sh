#!/usr/bin/env bash
# recipes/cython-cp311/build.sh — build cython 3.2.9 FROM SOURCE (generated).
#
...
echo "cython-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cython-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cython-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import Cython"
