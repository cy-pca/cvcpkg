#!/usr/bin/env bash
# recipes/pydantic-cp311/build.sh — build pydantic 2.13.4 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp311, hatch-fancy-pypi-readme-cp311, packaging-cp311, pathspec-cp311, pluggy-cp311, trove-classifiers-cp311, calver-cp311
...
echo "pydantic-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pydantic-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydantic-cp311: built $(basename "${WHEEL}")"
