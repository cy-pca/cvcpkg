...
# OWN HDF5 (recipes/hdf5, libhdf5.so.310), for the cp311 interpreter column of
...
#   <prefix>/lib/python3.11/site-packages/h5py/*.so
# so the rpath to reach <prefix>/lib is $ORIGIN/../../.. (three up: h5py ->
# site-packages -> python3.11 -> lib). We compute it per-file so the depth is
...
# (cython-cp311, pkgconfig-cp311, wheel-cp311, build-cp311) plus numpy-cp311 and
# the base python311 (pip + setuptools). This script does NOT re-solve that — it
...
# a cp311 default so the script is runnable standalone.
...
[ -e "${HDF5_DIR}/include/hdf5.h" ] || {
    echo "h5py-cp311: ${HDF5_DIR}/include/hdf5.h missing — is the hdf5 dep in the closure?" >&2
    exit 1
}
...
[ -n "${WHEEL}" ] || { echo "h5py-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
...
# site-packages scheme (PY_EXE decides the scheme -> lib/python3.11/site-packages).
...
[ -n "${H5PY_DIR}" ] || { echo "h5py-cp311: staged h5py/ not found under ${CVC_INSTALL_DIR}" >&2; exit 1; }
...
if [[ "${CVC_PLATFORM}" == "macos" ]]; then
    ...
    command -v install_name_tool >/dev/null 2>&1 || {
        echo "h5py-cp311: install_name_tool required on macOS but not found" >&2; exit 1; }
...
PY
    ...
    if ! command -v patchelf >/dev/null 2>&1; then
        echo "h5py-cp311: patchelf required on ${CVC_PLATFORM} but not found on PATH" >&2
        exit 1
    fi
    while IFS= read -r -d '' _so; do
