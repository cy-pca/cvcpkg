#!/usr/bin/env bash
# recipes/trimesh-cp312/build.sh — build trimesh 5.0.0 FROM SOURCE (generated).
#
...
echo "trimesh-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trimesh-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trimesh-cp312: built $(basename "${WHEEL}")"
