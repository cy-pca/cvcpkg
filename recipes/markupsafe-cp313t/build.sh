#!/usr/bin/env bash
# recipes/markupsafe-cp313t/build.sh — build MarkupSafe 3.0.3 FROM SOURCE for the cp313t
...
echo "markupsafe-cp313t: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools; print("markupsafe-cp313t: setuptools", setuptools.__version__, setuptools.__file__)'
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "markupsafe-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "markupsafe-cp313t: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "markupsafe-cp313t: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "markupsafe-cp313t: staged into ${SITE_PACKAGES}"
...
echo "markupsafe-cp313t: build + verification complete"
