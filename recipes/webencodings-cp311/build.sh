#!/usr/bin/env bash
# recipes/webencodings-cp311/build.sh — build webencodings 0.6.1 FROM SOURCE (generated).
#
...
echo "webencodings-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "webencodings-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "webencodings-cp311: built $(basename "${WHEEL}")"
