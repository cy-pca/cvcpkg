#!/usr/bin/env bash
# recipes/bcrypt-cp313t/build.sh — build bcrypt 5.0.0 FROM SOURCE for the cp313t
...
# cp313t, not abi3: the free-threaded interpreter has no stable ABI, and this
# default must agree with recipe.yaml — cvc_python_is_free_threaded keys off the
# trailing 't', so getting it wrong would silently skip the no-GIL assertion.
...
echo "bcrypt-cp313t: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, setuptools_rust, wheel; print("bcrypt-cp313t: setuptools", setuptools.__version__, "+ setuptools_rust + wheel")'
...
_require_prefix_tool() {
    ...
    [ -n "${resolved}" ] || {
        echo "bcrypt-cp313t: ${tool} not on PATH — is the 'rust' build dep in the closure?" >&2
        exit 1
    }
    case "${resolved}" in
        "${BLD}"/*|"${DEPS}"/*) echo "${resolved}" ;;
        *)
            echo "bcrypt-cp313t: refusing ${tool} at ${resolved} — it is outside the cvcpkg" >&2
...
echo "bcrypt-cp313t: cargo $("${CARGO_BIN}" --version) [${CARGO_BIN}]"
echo "bcrypt-cp313t: rustc $("${RUSTC_BIN}" --version) [${RUSTC_BIN}]"
...
if [ -n "${CVC_CARGO_OFFLINE:-}" ]; then
    ...
    echo "bcrypt-cp313t: CARGO_NET_OFFLINE=true — ${CARGO_HOME} must already contain"
    echo "  every crate in Cargo.lock, or cargo will fail rather than fetch."
else
    export CARGO_NET_OFFLINE=false
    echo "bcrypt-cp313t: NOTE — cargo will FETCH crates.io. This sdist ships"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "bcrypt-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "bcrypt-cp313t: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "bcrypt-cp313t: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "bcrypt-cp313t: staged into ${SITE_PACKAGES}"
...
echo "bcrypt-cp313t: build + verification complete"
