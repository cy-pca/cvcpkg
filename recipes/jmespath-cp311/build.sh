#!/usr/bin/env bash
# recipes/jmespath-cp311/build.sh — build jmespath 1.1.0 FROM SOURCE (generated).
#
...
echo "jmespath-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jmespath-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jmespath-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import jmespath"
