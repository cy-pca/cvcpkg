#!/usr/bin/env bash
# recipes/imageio-cp313/build.sh — build imageio 2.36.1 FROM SOURCE (generated).
#
...
echo "imageio-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "imageio-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "imageio-cp313: built $(basename "${WHEEL}")"
