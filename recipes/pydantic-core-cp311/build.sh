#!/usr/bin/env bash
# recipes/pydantic-core-cp311/build.sh — build pydantic-core 2.46.4 FROM SOURCE
# for the cp311 interpreter column of the per-interpreter wheel matrix.
#
...
echo "pydantic-core-cp311: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import maturin; print("pydantic-core-cp311: maturin backend", maturin.__file__)'
...
[ -n "${MATURIN_EXE}" ] || {
    echo "pydantic-core-cp311: no 'maturin' executable on PATH — the maturin-cp311" >&2
    echo "  build dep supplies it in ${BLD}/bin; is it in the closure?" >&2
    exit 1
}
echo "pydantic-core-cp311: maturin $("${MATURIN_EXE}" --version) [${MATURIN_EXE}]"
...
_require_prefix_tool() {
    ...
    [ -n "${resolved}" ] || {
        echo "pydantic-core-cp311: ${tool} not on PATH — is the 'rust' build dep in the closure?" >&2
        exit 1
    }
    case "${resolved}" in
        "${BLD}"/*|"${DEPS}"/*) echo "${resolved}" ;;
        *)
            echo "pydantic-core-cp311: refusing ${tool} at ${resolved} — it is outside the" >&2
...
echo "pydantic-core-cp311: cargo $("${CARGO_BIN}" --version) [${CARGO_BIN}]"
echo "pydantic-core-cp311: rustc $("${RUSTC_BIN}" --version) [${RUSTC_BIN}]"
...
if [ -n "${CVC_CARGO_OFFLINE:-}" ]; then
    ...
    echo "pydantic-core-cp311: CARGO_NET_OFFLINE=true — ${CARGO_HOME} must already"
    echo "  contain every crate in Cargo.lock, or cargo will fail rather than fetch."
else
    export CARGO_NET_OFFLINE=false
    echo "pydantic-core-cp311: NOTE — cargo will FETCH crates.io. This sdist ships"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "pydantic-core-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydantic-core-cp311: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "pydantic-core-cp311: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "pydantic-core-cp311: staged into ${SITE_PACKAGES}"
...
echo "pydantic-core-cp311: build + verification complete"
