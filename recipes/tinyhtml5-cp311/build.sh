#!/usr/bin/env bash
# recipes/tinyhtml5-cp311/build.sh — build tinyhtml5 2.1.0 FROM SOURCE (generated).
#
...
echo "tinyhtml5-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tinyhtml5-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tinyhtml5-cp311: built $(basename "${WHEEL}")"
