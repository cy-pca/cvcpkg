#!/usr/bin/env bash
# recipes/cython-cp313t/build.sh — build cython 3.2.9 FROM SOURCE (generated).
#
...
echo "cython-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cython-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cython-cp313t: built $(basename "${WHEEL}")"
