#!/usr/bin/env bash
# recipes/tqdm-cp312/build.sh — build tqdm 4.67.3 FROM SOURCE (generated).
#
...
echo "tqdm-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tqdm-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tqdm-cp312: built $(basename "${WHEEL}")"
