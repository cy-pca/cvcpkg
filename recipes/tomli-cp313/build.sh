#!/usr/bin/env bash
# recipes/tomli-cp313/build.sh — build tomli 2.4.1 FROM SOURCE (generated).
#
...
echo "tomli-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tomli-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tomli-cp313: built $(basename "${WHEEL}")"
