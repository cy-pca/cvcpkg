#!/usr/bin/env bash
# recipes/setuptools-cp313/build.sh — build setuptools 80.9.0 FROM SOURCE (generated).
#
...
echo "setuptools-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "setuptools-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "setuptools-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import setuptools"
