#!/usr/bin/env bash
# recipes/packaging-cp311/build.sh — build packaging 24.2 FROM SOURCE (generated).
#
...
echo "packaging-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "packaging-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "packaging-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import packaging"
