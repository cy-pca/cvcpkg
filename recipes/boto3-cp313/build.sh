#!/usr/bin/env bash
# recipes/boto3-cp313/build.sh — build boto3 1.43.49 FROM SOURCE (generated).
#
...
echo "boto3-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "boto3-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "boto3-cp313: built $(basename "${WHEEL}")"
