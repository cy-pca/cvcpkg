#!/usr/bin/env bash
# recipes/mypy-extensions-cp313/build.sh — build mypy-extensions 1.1.0 FROM SOURCE (generated).
#
...
echo "mypy-extensions-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mypy-extensions-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mypy-extensions-cp313: built $(basename "${WHEEL}")"
