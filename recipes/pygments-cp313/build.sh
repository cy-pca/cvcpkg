#!/usr/bin/env bash
# recipes/pygments-cp313/build.sh — build pygments 2.20.0 FROM SOURCE (generated).
#
...
echo "pygments-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pygments-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pygments-cp313: built $(basename "${WHEEL}")"
