#!/usr/bin/env bash
# recipes/httpx-cp313/build.sh — build httpx 0.28.1 FROM SOURCE (generated).
#
...
echo "httpx-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpx-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpx-cp313: built $(basename "${WHEEL}")"
