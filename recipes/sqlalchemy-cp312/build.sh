#!/usr/bin/env bash
# recipes/sqlalchemy-cp312/build.sh — build sqlalchemy 2.0.50 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, cython-cp312
...
echo "sqlalchemy-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "sqlalchemy-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "sqlalchemy-cp312: built $(basename "${WHEEL}")"
