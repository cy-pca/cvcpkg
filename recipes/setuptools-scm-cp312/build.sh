#!/usr/bin/env bash
# recipes/setuptools-scm-cp312/build.sh — build setuptools_scm 8.3.1 FROM SOURCE (sdist) for the
...
echo "setuptools-scm-cp312: building with ${PY} (python${PYMM})"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "setuptools-scm-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "setuptools-scm-cp312: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "setuptools-scm-cp312: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "setuptools-scm-cp312: staged into ${SITE_PACKAGES}"
...
echo "setuptools-scm-cp312: build + verification complete"
