#!/usr/bin/env bash
# recipes/pathspec-cp311/build.sh — build pathspec 1.1.1 FROM SOURCE (generated).
#
...
echo "pathspec-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pathspec-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pathspec-cp311: built $(basename "${WHEEL}")"
