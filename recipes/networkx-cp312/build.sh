#!/usr/bin/env bash
# recipes/networkx-cp312/build.sh — build networkx 3.4.2 FROM SOURCE (generated).
#
...
echo "networkx-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "networkx-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "networkx-cp312: built $(basename "${WHEEL}")"
