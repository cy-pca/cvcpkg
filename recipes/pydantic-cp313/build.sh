#!/usr/bin/env bash
# recipes/pydantic-cp313/build.sh — build pydantic 2.13.4 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp313, hatch-fancy-pypi-readme-cp313, packaging-cp313, pathspec-cp313, pluggy-cp313, trove-classifiers-cp313, calver-cp313
...
echo "pydantic-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pydantic-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydantic-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pydantic"
