#!/usr/bin/env bash
# recipes/hatch-fancy-pypi-readme-cp311/build.sh — build hatch-fancy-pypi-readme 24.1.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp311, packaging-cp311, pathspec-cp311, pluggy-cp311, trove-classifiers-cp311, calver-cp311
...
echo "hatch-fancy-pypi-readme-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatch-fancy-pypi-readme-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatch-fancy-pypi-readme-cp311: built $(basename "${WHEEL}")"
