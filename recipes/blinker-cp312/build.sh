#!/usr/bin/env bash
# recipes/blinker-cp312/build.sh — build blinker 1.9.0 FROM SOURCE (generated).
#
...
echo "blinker-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "blinker-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "blinker-cp312: built $(basename "${WHEEL}")"
