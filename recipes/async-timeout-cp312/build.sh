#!/usr/bin/env bash
# recipes/async-timeout-cp312/build.sh — build async-timeout 5.0.1 FROM SOURCE (generated).
#
...
echo "async-timeout-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "async-timeout-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "async-timeout-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import async_timeout"
