# recipes/pymupdf-cp311/build.ps1 — build PyMuPDF 1.28.2 FROM SOURCE on Windows
# for the cp311 column, against the MuPDF source fork PyMuPDF pins (approach (b);
...
$py = Get-CvcPythonExe
Write-Output "pymupdf-cp311: building with $py"
...
Write-Output "pymupdf-cp311: downloading $mupdfUrl"
...
if ($actual -ne $mupdfSha256.ToUpper()) {
    throw "pymupdf-cp311: MuPDF source sha256 mismatch`n  expected $mupdfSha256`n  actual   $actual"
}
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path $mupdfSrc)) { throw "pymupdf-cp311: MuPDF extract failed" }

# ── 2. Py2 -> Py3 fix in MuPDF's SWIG template (see build.sh) ───────────────
$swigPy = Join-Path $mupdfSrc 'scripts\wrap\swig.py'
if (-not (Test-Path $swigPy)) { throw "pymupdf-cp311: $swigPy not found — MuPDF layout changed?" }
...
if ($txt.Contains('PyString_FromString')) {
    ...
    Write-Output "pymupdf-cp311: patched PyString_FromString -> PyUnicode_FromString in swig.py"
}

# ── 3. Build-only backends: pipcl + libclang (version-pinned) ───────────────
& $py -c 'import pipcl, clang.cindex' 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Output "pymupdf-cp311: installing pinned build backends (pipcl==13, libclang==18.1.1)"
    & $py -m pip install --disable-pip-version-check --no-warn-script-location `
        --prefix $env:CVC_BUILD_PREFIX 'pipcl==13' 'libclang==18.1.1'
    if ($LASTEXITCODE -ne 0) { throw "pymupdf-cp311: pip install of build backends failed" }
}

# ── 4. SWIG from the catalog (host_tool) ────────────────────────────────────
$swig = (Get-Command swig -ErrorAction SilentlyContinue).Source
if (-not $swig) { throw "pymupdf-cp311: swig not on PATH — is the 'swig' host_tool in the closure?" }
...
if (-not ($env:SWIG_LIB -and (Test-Path (Join-Path $env:SWIG_LIB 'swig.swg')))) {
    throw "pymupdf-cp311: could not locate swig.swg under share\swig (SWIG_LIB=$($env:SWIG_LIB)); swig would fail with 'Unknown directive'."
}
Write-Output "pymupdf-cp311: swig [$swig], SWIG_LIB=$env:SWIG_LIB"

# ── 5. Build the wheel ──────────────────────────────────────────────────────
# PYMUPDF_SETUP_MUPDF_BUILD=<dir> -> use the local MuPDF tree, no download.
# PY_LIMITED_API=0 -> version-specific cp311 wheel (matches build.sh).
...
if ($LASTEXITCODE -ne 0) { throw "pymupdf-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter 'pymupdf-*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "pymupdf-cp311: no wheel produced under $wheelhouse" }
Write-Output "pymupdf-cp311: built $($wheel.Name)"
...
if ($LASTEXITCODE -ne 0) { throw "pymupdf-cp311: pip install failed ($LASTEXITCODE)" }
...
print('pymupdf-cp311: rasterised page0 ->', pix.width, 'x', pix.height, 'n=', pix.n, 'PNG', len(png), 'bytes')
"@
