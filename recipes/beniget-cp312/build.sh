#!/usr/bin/env bash
# recipes/beniget-cp312/build.sh — build beniget 0.5.0 FROM SOURCE (generated).
#
...
echo "beniget-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "beniget-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "beniget-cp312: built $(basename "${WHEEL}")"
