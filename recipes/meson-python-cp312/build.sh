#!/usr/bin/env bash
# recipes/meson-python-cp312/build.sh — build meson-python 0.18.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: meson, packaging-cp312, pyproject-metadata-cp312
...
echo "meson-python-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "meson-python-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "meson-python-cp312: built $(basename "${WHEEL}")"
