#!/usr/bin/env bash
# recipes/msal-cp311/build.sh — build msal 1.37.0 FROM SOURCE (generated).
#
...
echo "msal-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "msal-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "msal-cp311: built $(basename "${WHEEL}")"
