#!/usr/bin/env bash
# recipes/colorama-cp312/build.sh — build colorama 0.4.6 FROM SOURCE (generated).
#
...
echo "colorama-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "colorama-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "colorama-cp312: built $(basename "${WHEEL}")"
