#!/usr/bin/env bash
# recipes/itsdangerous-cp312/build.sh — build itsdangerous 2.2.0 FROM SOURCE (generated).
#
...
echo "itsdangerous-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "itsdangerous-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "itsdangerous-cp312: built $(basename "${WHEEL}")"
