#!/usr/bin/env bash
# recipes/attrs-cp311/build.sh — build attrs 26.1.0 FROM SOURCE (generated).
#
...
echo "attrs-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "attrs-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "attrs-cp311: built $(basename "${WHEEL}")"
