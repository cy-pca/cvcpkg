#!/usr/bin/env bash
# recipes/isodate-cp313/build.sh — build isodate 0.7.2 FROM SOURCE (generated).
#
...
echo "isodate-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "isodate-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "isodate-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import isodate"
