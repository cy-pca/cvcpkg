#!/usr/bin/env bash
# recipes/fsspec-cp311/build.sh — build fsspec 2024.10.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp311, hatch-vcs-cp311, packaging-cp311, pathspec-cp311, pluggy-cp311, trove-classifiers-cp311, calver-cp311
...
echo "fsspec-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fsspec-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fsspec-cp311: built $(basename "${WHEEL}")"
