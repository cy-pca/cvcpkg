#!/usr/bin/env bash
# recipes/jsonschema-cp311/build.sh — build jsonschema 4.17.3 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp311, hatch-vcs-cp311, hatch-fancy-pypi-readme-cp311, packaging-cp311, pathspec-cp311, pluggy-cp311, trove-classifiers-cp311, calver-cp311
...
echo "jsonschema-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jsonschema-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jsonschema-cp311: built $(basename "${WHEEL}")"
