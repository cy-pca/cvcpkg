#!/usr/bin/env bash
# recipes/pyjwt-cp312/build.sh — build pyjwt 2.13.0 FROM SOURCE (generated).
#
...
echo "pyjwt-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyjwt-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyjwt-cp312: built $(basename "${WHEEL}")"
