#!/usr/bin/env bash
# recipes/pefile-cp311/build.sh — build pefile 2024.8.26 FROM SOURCE (generated).
#
...
echo "pefile-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pefile-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pefile-cp311: built $(basename "${WHEEL}")"
