#!/usr/bin/env bash
# recipes/urllib3-cp312/build.sh — build urllib3 2.7.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp312, hatch-vcs-cp312, setuptools-scm-cp312, packaging-cp312, pathspec-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "urllib3-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "urllib3-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "urllib3-cp312: built $(basename "${WHEEL}")"
