#!/usr/bin/env bash
# recipes/colorama-cp313/build.sh — build colorama 0.4.6 FROM SOURCE (generated).
#
...
echo "colorama-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "colorama-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "colorama-cp313: built $(basename "${WHEEL}")"
