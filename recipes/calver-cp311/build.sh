#!/usr/bin/env bash
# recipes/calver-cp311/build.sh — build calver 2022.6.26 FROM SOURCE (generated).
#
...
echo "calver-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "calver-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "calver-cp311: built $(basename "${WHEEL}")"
