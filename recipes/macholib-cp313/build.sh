#!/usr/bin/env bash
# recipes/macholib-cp313/build.sh — build macholib 1.16.4 FROM SOURCE (generated).
#
...
echo "macholib-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "macholib-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "macholib-cp313: built $(basename "${WHEEL}")"
