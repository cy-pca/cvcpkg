#!/usr/bin/env bash
# recipes/azure-identity-cp313/build.sh — build azure-identity 1.25.3 FROM SOURCE (generated).
#
...
echo "azure-identity-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "azure-identity-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "azure-identity-cp313: built $(basename "${WHEEL}")"
