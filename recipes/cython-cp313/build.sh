#!/usr/bin/env bash
# recipes/cython-cp313/build.sh — build cython 3.2.9 FROM SOURCE (generated).
#
...
echo "cython-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cython-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cython-cp313: built $(basename "${WHEEL}")"
