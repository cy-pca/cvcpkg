#!/usr/bin/env bash
# recipes/pyproject-metadata-cp311/build.sh — build pyproject-metadata 0.9.1 FROM SOURCE (generated).
#
...
echo "pyproject-metadata-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyproject-metadata-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyproject-metadata-cp311: built $(basename "${WHEEL}")"
