#!/usr/bin/env bash
# recipes/pymysql-cp311/build.sh — build pymysql 1.2.0 FROM SOURCE (generated).
#
...
echo "pymysql-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pymysql-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pymysql-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pymysql"
