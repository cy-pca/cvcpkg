#!/usr/bin/env bash
# recipes/itsdangerous-cp313t/build.sh — build itsdangerous 2.2.0 FROM SOURCE (generated).
#
...
echo "itsdangerous-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "itsdangerous-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "itsdangerous-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import itsdangerous"
