#!/usr/bin/env bash
# recipes/meson-python-cp313/build.sh — build meson-python 0.18.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: meson, packaging-cp313, pyproject-metadata-cp313
...
echo "meson-python-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "meson-python-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "meson-python-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import mesonpy"
