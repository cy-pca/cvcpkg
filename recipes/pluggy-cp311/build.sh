#!/usr/bin/env bash
# recipes/pluggy-cp311/build.sh — build pluggy 1.6.0 FROM SOURCE (generated).
#
...
echo "pluggy-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pluggy-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pluggy-cp311: built $(basename "${WHEEL}")"
