#!/usr/bin/env bash
# recipes/pytest-cp312/build.sh — build pytest 9.1.1 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, setuptools-scm-cp312, trove-classifiers-cp312
...
echo "pytest-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pytest-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pytest-cp312: built $(basename "${WHEEL}")"
