#!/usr/bin/env bash
# recipes/pluggy-cp312/build.sh — build pluggy 1.6.0 FROM SOURCE (generated).
#
...
echo "pluggy-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pluggy-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pluggy-cp312: built $(basename "${WHEEL}")"
