#!/usr/bin/env bash
# recipes/googleapis-common-protos-cp311/build.sh — build googleapis-common-protos 1.75.0 FROM SOURCE (generated).
#
...
echo "googleapis-common-protos-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "googleapis-common-protos-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "googleapis-common-protos-cp311: built $(basename "${WHEEL}")"
