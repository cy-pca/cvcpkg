#!/usr/bin/env bash
# recipes/cppy-cp311/build.sh — build cppy 1.3.1 FROM SOURCE (generated).
#
...
echo "cppy-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cppy-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cppy-cp311: built $(basename "${WHEEL}")"
