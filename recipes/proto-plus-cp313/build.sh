#!/usr/bin/env bash
# recipes/proto-plus-cp313/build.sh — build proto-plus 1.28.1 FROM SOURCE (generated).
#
...
echo "proto-plus-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "proto-plus-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "proto-plus-cp313: built $(basename "${WHEEL}")"
