#!/usr/bin/env bash
# recipes/requests-cp311/build.sh — build requests 2.34.2 FROM SOURCE (generated).
#
...
echo "requests-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "requests-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "requests-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import requests"
