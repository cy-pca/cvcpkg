#!/usr/bin/env bash
# recipes/numpy-cp313t/build.sh — build NumPy 2.4.6 FROM SOURCE (meson-python)
# against cvcpkg's OpenBLAS (LP64), then install the resulting wheel into the
# python313t (free-threaded, no-GIL) interpreter's site-packages.
#
...
PY="$(cvc_python_exe)"                         # <CVC_DEPS_PREFIX>/bin/python3.13t
DEPS="${CVC_DEPS_PREFIX:-${CVC_INSTALL_DIR}}"
BLD="${CVC_BUILD_PREFIX:-${DEPS}}"
# The free-threaded interpreter's site-packages is lib/python3.13t/, WITH the
# 't' — see cython-cp313t's package.files. The sibling columns derive this from
# CVC_PYTHON_ABI and strip the 't' ("${_D%t}"), which is a harmless no-op for
# cp311/cp312/cp313 but would point this column at lib/python3.13/ and leave
# meson-python and Cython unimportable. cvc_interp_version keeps the suffix.
_PYMM="$(cvc_interp_version "${CVC_PYTHON_INTERPRETER:-python313t}")"   # 3.13t
...
case "${CVC_PLATFORM}" in
  *)
    ...
    [ -n "${PKG_CONFIG:-}" ] || { echo "numpy-cp313t: no pkg-config found in ${BLD}/bin, ${DEPS}/bin or PATH" >&2; exit 1; }
    ...
    if ! "${PKG_CONFIG}" --exists openblas; then
        echo "numpy-cp313t: openblas.pc not found by ${PKG_CONFIG}" >&2
        echo "  PKG_CONFIG_LIBDIR=${PKG_CONFIG_LIBDIR}" >&2
        ls -la "${DEPS}/lib/pkgconfig" "${BLD}/lib/pkgconfig" 2>&1 | head -40 >&2
        exit 1
    fi
    echo "numpy-cp313t: openblas $("${PKG_CONFIG}" --modversion openblas) via ${PKG_CONFIG}"
    BLAS_ARGS+=(
...
[ -n "${WHEEL}" ] || { echo "numpy-cp313t: no wheel produced" >&2; exit 1; }
echo "numpy-cp313t: built $(basename "${WHEEL}")"
...
[ -n "${NP_DIR}" ] || { echo "numpy-cp313t: staged numpy/ not found" >&2; exit 1; }
...
if [ "${CVC_PLATFORM}" != "macos" ]; then
  command -v patchelf >/dev/null 2>&1 || { echo "numpy-cp313t: patchelf missing" >&2; exit 1; }
...
np.show_config()
print("numpy-cp313t build + verification complete")
PYCHECK
