#!/usr/bin/env bash
# recipes/fsspec-cp313t/build.sh — build fsspec 2024.10.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp313t, hatch-vcs-cp313t, packaging-cp313t, pathspec-cp313t, pluggy-cp313t, trove-classifiers-cp313t, calver-cp313t
...
echo "fsspec-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fsspec-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fsspec-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import fsspec"
