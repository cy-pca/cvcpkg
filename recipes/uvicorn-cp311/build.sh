#!/usr/bin/env bash
# recipes/uvicorn-cp311/build.sh — build uvicorn 0.47.0 FROM SOURCE (generated).
#
...
echo "uvicorn-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "uvicorn-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "uvicorn-cp311: built $(basename "${WHEEL}")"
