#!/usr/bin/env bash
# recipes/pywin32-ctypes-cp311/build.sh — build pywin32-ctypes 0.2.3 FROM SOURCE (generated).
#
...
echo "pywin32-ctypes-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pywin32-ctypes-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pywin32-ctypes-cp311: built $(basename "${WHEEL}")"
