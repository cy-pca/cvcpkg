#!/usr/bin/env bash
# recipes/calver-cp312/build.sh — build calver 2022.6.26 FROM SOURCE (generated).
#
...
echo "calver-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "calver-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "calver-cp312: built $(basename "${WHEEL}")"
