#!/usr/bin/env bash
# recipes/kiwisolver-cp313/build.sh — build kiwisolver 1.4.8 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, setuptools-scm-cp313, cppy-cp313
...
echo "kiwisolver-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "kiwisolver-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "kiwisolver-cp313: built $(basename "${WHEEL}")"
