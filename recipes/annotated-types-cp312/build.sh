#!/usr/bin/env bash
# recipes/annotated-types-cp312/build.sh — build annotated-types 0.7.0 FROM SOURCE (generated).
#
...
echo "annotated-types-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "annotated-types-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "annotated-types-cp312: built $(basename "${WHEEL}")"
