#!/usr/bin/env bash
# recipes/pynacl-cp313/build.sh — build pynacl 1.6.2 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, wheel-cp313, cffi-cp313, pycparser-cp313
...
echo "pynacl-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pynacl-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pynacl-cp313: built $(basename "${WHEEL}")"
