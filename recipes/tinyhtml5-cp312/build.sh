#!/usr/bin/env bash
# recipes/tinyhtml5-cp312/build.sh — build tinyhtml5 2.1.0 FROM SOURCE (generated).
#
...
echo "tinyhtml5-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "tinyhtml5-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "tinyhtml5-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import tinyhtml5"
