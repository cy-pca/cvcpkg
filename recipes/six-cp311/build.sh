#!/usr/bin/env bash
# recipes/six-cp311/build.sh — build six 1.17.0 FROM SOURCE (generated).
#
...
echo "six-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "six-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "six-cp311: built $(basename "${WHEEL}")"
