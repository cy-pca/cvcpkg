#!/usr/bin/env bash
# recipes/pyphen-cp313/build.sh — build pyphen 0.18.1 FROM SOURCE (generated).
#
...
echo "pyphen-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyphen-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyphen-cp313: built $(basename "${WHEEL}")"
