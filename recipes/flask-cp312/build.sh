#!/usr/bin/env bash
# recipes/flask-cp312/build.sh — build flask 3.1.3 FROM SOURCE (generated).
#
...
echo "flask-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "flask-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "flask-cp312: built $(basename "${WHEEL}")"
