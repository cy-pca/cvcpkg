#!/usr/bin/env bash
# recipes/protobuf-cp313/build.sh — build protobuf 7.35.1 FROM SOURCE (generated).
#
...
echo "protobuf-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "protobuf-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "protobuf-cp313: built $(basename "${WHEEL}")"
