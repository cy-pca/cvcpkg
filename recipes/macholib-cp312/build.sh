#!/usr/bin/env bash
# recipes/macholib-cp312/build.sh — build macholib 1.16.4 FROM SOURCE (generated).
#
...
echo "macholib-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "macholib-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "macholib-cp312: built $(basename "${WHEEL}")"
