#!/usr/bin/env bash
# recipes/sqlalchemy-cp313/build.sh — build sqlalchemy 2.0.50 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, cython-cp313
...
echo "sqlalchemy-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "sqlalchemy-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "sqlalchemy-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import sqlalchemy"
