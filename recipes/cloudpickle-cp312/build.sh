#!/usr/bin/env bash
# recipes/cloudpickle-cp312/build.sh — build cloudpickle 3.1.2 FROM SOURCE (generated).
#
...
echo "cloudpickle-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cloudpickle-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cloudpickle-cp312: built $(basename "${WHEEL}")"
