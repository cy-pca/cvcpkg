#!/usr/bin/env bash
# recipes/itsdangerous-cp311/build.sh — build itsdangerous 2.2.0 FROM SOURCE (generated).
#
...
echo "itsdangerous-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "itsdangerous-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "itsdangerous-cp311: built $(basename "${WHEEL}")"
