#!/usr/bin/env bash
# recipes/requests-cp313/build.sh — build requests 2.34.2 FROM SOURCE (generated).
#
...
echo "requests-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "requests-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "requests-cp313: built $(basename "${WHEEL}")"
