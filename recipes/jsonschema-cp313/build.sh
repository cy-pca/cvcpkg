#!/usr/bin/env bash
# recipes/jsonschema-cp313/build.sh — build jsonschema 4.17.3 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp313, hatch-vcs-cp313, hatch-fancy-pypi-readme-cp313, packaging-cp313, pathspec-cp313, pluggy-cp313, trove-classifiers-cp313, calver-cp313
...
echo "jsonschema-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jsonschema-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jsonschema-cp313: built $(basename "${WHEEL}")"
