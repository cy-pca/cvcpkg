#!/usr/bin/env bash
# recipes/idna-cp313/build.sh — build idna 3.16 FROM SOURCE (generated).
#
...
echo "idna-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "idna-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "idna-cp313: built $(basename "${WHEEL}")"
