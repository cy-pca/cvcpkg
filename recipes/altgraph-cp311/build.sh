#!/usr/bin/env bash
# recipes/altgraph-cp311/build.sh — build altgraph 0.17.5 FROM SOURCE (generated).
#
...
echo "altgraph-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "altgraph-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "altgraph-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import altgraph"
