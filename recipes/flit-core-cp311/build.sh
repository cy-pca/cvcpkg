#!/usr/bin/env bash
# recipes/flit-core-cp311/build.sh — build flit-core 3.12.0 FROM SOURCE (generated).
#
...
echo "flit-core-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "flit-core-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "flit-core-cp311: built $(basename "${WHEEL}")"
