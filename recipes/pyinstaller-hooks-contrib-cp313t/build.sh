#!/usr/bin/env bash
# recipes/pyinstaller-hooks-contrib-cp313t/build.sh — build pyinstaller-hooks-contrib 2026.6 FROM SOURCE (generated).
#
...
echo "pyinstaller-hooks-contrib-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyinstaller-hooks-contrib-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyinstaller-hooks-contrib-cp313t: built $(basename "${WHEEL}")"
