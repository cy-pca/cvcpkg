#!/usr/bin/env bash
# recipes/macholib-cp311/build.sh — build macholib 1.16.4 FROM SOURCE (generated).
#
...
echo "macholib-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "macholib-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "macholib-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import macholib"
