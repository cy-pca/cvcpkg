#!/usr/bin/env bash
# recipes/mako-cp313/build.sh — build mako 1.3.12 FROM SOURCE (generated).
#
...
echo "mako-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mako-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mako-cp313: built $(basename "${WHEEL}")"
