#!/usr/bin/env bash
# recipes/filelock-cp313/build.sh — build filelock 3.16.1 FROM SOURCE (generated).
#
...
echo "filelock-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "filelock-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "filelock-cp313: built $(basename "${WHEEL}")"
