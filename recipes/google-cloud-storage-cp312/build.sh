#!/usr/bin/env bash
# recipes/google-cloud-storage-cp312/build.sh — build google-cloud-storage 3.13.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, wheel-cp312
...
echo "google-cloud-storage-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-cloud-storage-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-cloud-storage-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import google.cloud.storage"
