#!/usr/bin/env bash
# recipes/httpx-cp312/build.sh — build httpx 0.28.1 FROM SOURCE (generated).
#
...
echo "httpx-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "httpx-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "httpx-cp312: built $(basename "${WHEEL}")"
