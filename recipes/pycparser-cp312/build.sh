#!/usr/bin/env bash
# recipes/pycparser-cp312/build.sh — build pycparser 3.0 FROM SOURCE (generated).
#
...
echo "pycparser-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pycparser-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pycparser-cp312: built $(basename "${WHEEL}")"
