#!/usr/bin/env bash
# recipes/mako-cp311/build.sh — build mako 1.3.12 FROM SOURCE (generated).
#
...
echo "mako-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mako-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mako-cp311: built $(basename "${WHEEL}")"
