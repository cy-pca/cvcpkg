#!/usr/bin/env bash
# recipes/mpmath-cp311/build.sh — build mpmath 1.3.0 FROM SOURCE (generated).
#
...
echo "mpmath-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mpmath-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mpmath-cp311: built $(basename "${WHEEL}")"
