#!/usr/bin/env bash
# recipes/google-cloud-storage-cp313/build.sh — build google-cloud-storage 3.13.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, wheel-cp313
...
echo "google-cloud-storage-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-cloud-storage-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-cloud-storage-cp313: built $(basename "${WHEEL}")"
