#!/usr/bin/env bash
# recipes/trimesh-cp313t/build.sh — build trimesh 5.0.0 FROM SOURCE (generated).
#
...
echo "trimesh-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trimesh-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trimesh-cp313t: built $(basename "${WHEEL}")"
