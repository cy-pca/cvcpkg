#!/usr/bin/env bash
# recipes/cycler-cp313/build.sh — build cycler 0.12.1 FROM SOURCE (generated).
#
...
echo "cycler-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cycler-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cycler-cp313: built $(basename "${WHEEL}")"
