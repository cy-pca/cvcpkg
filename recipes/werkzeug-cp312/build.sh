#!/usr/bin/env bash
# recipes/werkzeug-cp312/build.sh — build werkzeug 3.1.8 FROM SOURCE (generated).
#
...
echo "werkzeug-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "werkzeug-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "werkzeug-cp312: built $(basename "${WHEEL}")"
