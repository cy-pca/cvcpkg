#!/usr/bin/env bash
# recipes/pycparser-cp313/build.sh — build pycparser 3.0 FROM SOURCE (generated).
#
...
echo "pycparser-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pycparser-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pycparser-cp313: built $(basename "${WHEEL}")"
