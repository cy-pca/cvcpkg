#!/usr/bin/env bash
# recipes/urllib3-cp311/build.sh — build urllib3 2.7.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp311, hatch-vcs-cp311, setuptools-scm-cp311, packaging-cp311, pathspec-cp311, pluggy-cp311, trove-classifiers-cp311, calver-cp311
...
echo "urllib3-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "urllib3-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "urllib3-cp311: built $(basename "${WHEEL}")"
