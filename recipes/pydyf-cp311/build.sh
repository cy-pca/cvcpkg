#!/usr/bin/env bash
# recipes/pydyf-cp311/build.sh — build pydyf 0.12.1 FROM SOURCE (generated).
#
...
echo "pydyf-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pydyf-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydyf-cp311: built $(basename "${WHEEL}")"
