#!/usr/bin/env bash
# recipes/click-cp313/build.sh — build click 8.4.1 FROM SOURCE (generated).
#
...
echo "click-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "click-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "click-cp313: built $(basename "${WHEEL}")"
