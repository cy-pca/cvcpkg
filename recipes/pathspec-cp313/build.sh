#!/usr/bin/env bash
# recipes/pathspec-cp313/build.sh — build pathspec 1.1.1 FROM SOURCE (generated).
#
...
echo "pathspec-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pathspec-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pathspec-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pathspec"
