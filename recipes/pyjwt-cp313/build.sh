#!/usr/bin/env bash
# recipes/pyjwt-cp313/build.sh — build pyjwt 2.13.0 FROM SOURCE (generated).
#
...
echo "pyjwt-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyjwt-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyjwt-cp313: built $(basename "${WHEEL}")"
