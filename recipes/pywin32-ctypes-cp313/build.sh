#!/usr/bin/env bash
# recipes/pywin32-ctypes-cp313/build.sh — build pywin32-ctypes 0.2.3 FROM SOURCE (generated).
#
...
echo "pywin32-ctypes-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pywin32-ctypes-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pywin32-ctypes-cp313: built $(basename "${WHEEL}")"
