#!/usr/bin/env bash
# recipes/anyio-cp313/build.sh — build anyio 4.13.0 FROM SOURCE (generated).
#
...
echo "anyio-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "anyio-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "anyio-cp313: built $(basename "${WHEEL}")"
