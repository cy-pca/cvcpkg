#!/usr/bin/env bash
# recipes/paramiko-cp311/build.sh — build paramiko 5.0.0 FROM SOURCE (generated).
#
...
echo "paramiko-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "paramiko-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "paramiko-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import paramiko"
