#!/usr/bin/env bash
# recipes/alembic-cp311/build.sh — build alembic 1.18.4 FROM SOURCE (generated).
#
...
echo "alembic-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "alembic-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "alembic-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import alembic"
