#!/usr/bin/env bash
# recipes/hatch-vcs-cp313/build.sh — build hatch-vcs 0.4.0 FROM SOURCE (generated).
#
...
echo "hatch-vcs-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatch-vcs-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatch-vcs-cp313: built $(basename "${WHEEL}")"
