#!/usr/bin/env bash
# recipes/pyproject-metadata-cp312/build.sh — build pyproject-metadata 0.9.1 FROM SOURCE (generated).
#
...
echo "pyproject-metadata-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyproject-metadata-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyproject-metadata-cp312: built $(basename "${WHEEL}")"
