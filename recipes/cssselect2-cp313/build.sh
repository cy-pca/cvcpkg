#!/usr/bin/env bash
# recipes/cssselect2-cp313/build.sh — build cssselect2 0.9.0 FROM SOURCE (generated).
#
...
echo "cssselect2-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cssselect2-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cssselect2-cp313: built $(basename "${WHEEL}")"
