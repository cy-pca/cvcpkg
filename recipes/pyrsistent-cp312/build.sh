#!/usr/bin/env bash
# recipes/pyrsistent-cp312/build.sh — build pyrsistent 0.20.0 FROM SOURCE (generated).
#
...
echo "pyrsistent-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyrsistent-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyrsistent-cp312: built $(basename "${WHEEL}")"
