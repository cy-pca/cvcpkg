#!/usr/bin/env bash
# recipes/farama-notifications-cp312/build.sh — build farama-notifications 0.0.6 FROM SOURCE (generated).
#
...
echo "farama-notifications-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "farama-notifications-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "farama-notifications-cp312: built $(basename "${WHEEL}")"
