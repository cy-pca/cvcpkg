#!/usr/bin/env bash
# recipes/trimesh-cp313/build.sh — build trimesh 5.0.0 FROM SOURCE (generated).
#
...
echo "trimesh-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trimesh-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trimesh-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import trimesh"
