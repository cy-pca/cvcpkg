#!/usr/bin/env bash
# recipes/webencodings-cp313/build.sh — build webencodings 0.6.1 FROM SOURCE (generated).
#
...
echo "webencodings-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "webencodings-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "webencodings-cp313: built $(basename "${WHEEL}")"
