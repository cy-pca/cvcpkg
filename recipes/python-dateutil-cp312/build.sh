#!/usr/bin/env bash
# recipes/python-dateutil-cp312/build.sh — build python-dateutil 2.9.0.post0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp312, wheel-cp312, setuptools-scm-cp312
...
echo "python-dateutil-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "python-dateutil-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "python-dateutil-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import dateutil"
