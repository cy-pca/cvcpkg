#!/usr/bin/env bash
# recipes/click-cp312/build.sh — build click 8.4.1 FROM SOURCE (generated).
#
...
echo "click-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "click-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "click-cp312: built $(basename "${WHEEL}")"
