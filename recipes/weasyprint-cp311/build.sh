#!/usr/bin/env bash
# recipes/weasyprint-cp311/build.sh — build weasyprint 69.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: flit-core-cp311, pycparser-cp311
...
echo "weasyprint-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "weasyprint-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "weasyprint-cp311: built $(basename "${WHEEL}")"
