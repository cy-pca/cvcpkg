#!/usr/bin/env bash
# recipes/pythran-cp313t/build.sh — build pythran 0.18.1 FROM SOURCE (generated).
#
...
echo "pythran-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pythran-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pythran-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pythran"
