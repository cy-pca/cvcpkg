#!/usr/bin/env bash
# recipes/pynacl-cp311/build.sh — build pynacl 1.6.2 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311, wheel-cp311, cffi-cp311, pycparser-cp311
...
echo "pynacl-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pynacl-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pynacl-cp311: built $(basename "${WHEEL}")"
