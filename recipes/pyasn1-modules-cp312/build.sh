#!/usr/bin/env bash
# recipes/pyasn1-modules-cp312/build.sh — build pyasn1-modules 0.4.2 FROM SOURCE (generated).
#
...
echo "pyasn1-modules-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyasn1-modules-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyasn1-modules-cp312: built $(basename "${WHEEL}")"
