#!/usr/bin/env bash
# recipes/hatch-vcs-cp311/build.sh — build hatch-vcs 0.4.0 FROM SOURCE (generated).
#
...
echo "hatch-vcs-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatch-vcs-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatch-vcs-cp311: built $(basename "${WHEEL}")"
