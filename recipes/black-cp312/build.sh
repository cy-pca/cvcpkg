#!/usr/bin/env bash
# recipes/black-cp312/build.sh — build black 26.5.1 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatch-fancy-pypi-readme-cp312, hatch-vcs-cp312, hatchling-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "black-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "black-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "black-cp312: built $(basename "${WHEEL}")"
