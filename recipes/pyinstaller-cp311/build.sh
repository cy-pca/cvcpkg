#!/usr/bin/env bash
# recipes/pyinstaller-cp311/build.sh — build PyInstaller 6.21.0 FROM SOURCE for the cp311
# interpreter column.
#
...
echo "pyinstaller-cp311: building against ${PY_EXE}"
...
export PYTHONPATH="${BLD}/lib/python3.11/site-packages${PYTHONPATH:+:${PYTHONPATH}}"
...
[ -n "${WHEEL}" ] || { echo "pyinstaller-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyinstaller-cp311: built $(basename "${WHEEL}")"
...
echo "pyinstaller-cp311: build + verification complete"
