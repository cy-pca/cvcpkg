#!/usr/bin/env bash
# recipes/weasyprint-cp313/build.sh — build weasyprint 69.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: flit-core-cp313, pycparser-cp313
...
echo "weasyprint-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "weasyprint-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "weasyprint-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "from weasyprint import HTML; assert HTML(string='<p>x</p>').write_pdf()[:4] == b'%PDF'"
