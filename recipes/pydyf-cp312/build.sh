#!/usr/bin/env bash
# recipes/pydyf-cp312/build.sh — build pydyf 0.12.1 FROM SOURCE (generated).
#
...
echo "pydyf-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pydyf-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pydyf-cp312: built $(basename "${WHEEL}")"
