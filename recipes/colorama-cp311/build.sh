#!/usr/bin/env bash
# recipes/colorama-cp311/build.sh — build colorama 0.4.6 FROM SOURCE (generated).
#
...
echo "colorama-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "colorama-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "colorama-cp311: built $(basename "${WHEEL}")"
