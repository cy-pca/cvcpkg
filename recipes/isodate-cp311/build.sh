#!/usr/bin/env bash
# recipes/isodate-cp311/build.sh — build isodate 0.7.2 FROM SOURCE (generated).
#
...
echo "isodate-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "isodate-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "isodate-cp311: built $(basename "${WHEEL}")"
