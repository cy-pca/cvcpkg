#!/usr/bin/env bash
# recipes/botocore-cp313/build.sh — build botocore 1.43.49 FROM SOURCE (generated).
#
...
echo "botocore-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "botocore-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "botocore-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import botocore"
