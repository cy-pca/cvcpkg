#!/usr/bin/env bash
# recipes/protobuf-cp312/build.sh — build protobuf 7.35.1 FROM SOURCE (generated).
#
...
echo "protobuf-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "protobuf-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "protobuf-cp312: built $(basename "${WHEEL}")"
