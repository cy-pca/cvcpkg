#!/usr/bin/env bash
# recipes/jmespath-cp312/build.sh — build jmespath 1.1.0 FROM SOURCE (generated).
#
...
echo "jmespath-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jmespath-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jmespath-cp312: built $(basename "${WHEEL}")"
