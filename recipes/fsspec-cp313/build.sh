#!/usr/bin/env bash
# recipes/fsspec-cp313/build.sh — build fsspec 2024.10.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatchling-cp313, hatch-vcs-cp313, packaging-cp313, pathspec-cp313, pluggy-cp313, trove-classifiers-cp313, calver-cp313
...
echo "fsspec-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "fsspec-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "fsspec-cp313: built $(basename "${WHEEL}")"
