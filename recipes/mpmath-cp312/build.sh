#!/usr/bin/env bash
# recipes/mpmath-cp312/build.sh — build mpmath 1.3.0 FROM SOURCE (generated).
#
...
echo "mpmath-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "mpmath-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "mpmath-cp312: built $(basename "${WHEEL}")"
