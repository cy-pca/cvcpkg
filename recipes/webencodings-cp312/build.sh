#!/usr/bin/env bash
# recipes/webencodings-cp312/build.sh — build webencodings 0.6.1 FROM SOURCE (generated).
#
...
echo "webencodings-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "webencodings-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "webencodings-cp312: built $(basename "${WHEEL}")"
