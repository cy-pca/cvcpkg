#!/usr/bin/env bash
# recipes/pybind11-cp311/build.sh — build pybind11 2.13.6 FROM SOURCE (generated).
#
...
echo "pybind11-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pybind11-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pybind11-cp311: built $(basename "${WHEEL}")"
