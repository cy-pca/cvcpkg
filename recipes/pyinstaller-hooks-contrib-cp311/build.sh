#!/usr/bin/env bash
# recipes/pyinstaller-hooks-contrib-cp311/build.sh — build pyinstaller-hooks-contrib 2026.6 FROM SOURCE (generated).
#
...
echo "pyinstaller-hooks-contrib-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyinstaller-hooks-contrib-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyinstaller-hooks-contrib-cp311: built $(basename "${WHEEL}")"
