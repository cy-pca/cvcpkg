#!/usr/bin/env bash
# recipes/google-api-core-cp311/build.sh — build google-api-core 2.31.0 FROM SOURCE (generated).
#
...
echo "google-api-core-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-api-core-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-api-core-cp311: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import google.api_core"
