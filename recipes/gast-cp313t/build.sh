#!/usr/bin/env bash
# recipes/gast-cp313t/build.sh — build gast 0.7.0 FROM SOURCE (generated).
#
...
echo "gast-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gast-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gast-cp313t: built $(basename "${WHEEL}")"
