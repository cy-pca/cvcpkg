#!/usr/bin/env bash
# recipes/cloudpickle-cp313t/build.sh — build cloudpickle 3.1.2 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: flit-core-cp313t
...
echo "cloudpickle-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cloudpickle-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cloudpickle-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import cloudpickle, pickle; f = pickle.loads(cloudpickle.dumps(lambda x: x + 1)); assert f(1) == 2"
