#!/usr/bin/env bash
# recipes/google-api-core-cp313/build.sh — build google-api-core 2.31.0 FROM SOURCE (generated).
#
...
echo "google-api-core-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-api-core-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-api-core-cp313: built $(basename "${WHEEL}")"
