#!/usr/bin/env bash
# recipes/idna-cp311/build.sh — build idna 3.16 FROM SOURCE (generated).
#
...
echo "idna-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "idna-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "idna-cp311: built $(basename "${WHEEL}")"
