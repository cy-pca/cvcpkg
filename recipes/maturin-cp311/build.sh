#!/usr/bin/env bash
# recipes/maturin-cp311/build.sh — build maturin 1.14.1 FROM SOURCE for the
# cp311 interpreter column.
#
...
echo "maturin-cp311: building with ${PY} (python${PYMM})"
...
"${PY}" -c 'import setuptools, setuptools_rust, wheel; print("maturin-cp311: setuptools", setuptools.__version__, "+ setuptools_rust + wheel")'
...
_require_prefix_tool() {
    ...
    [ -n "${resolved}" ] || {
        echo "maturin-cp311: ${tool} not on PATH — is the 'rust' build dep in the closure?" >&2
        exit 1
    }
    case "${resolved}" in
        "${BLD}"/*|"${DEPS}"/*) echo "${resolved}" ;;
        *)
            echo "maturin-cp311: refusing ${tool} at ${resolved} — it is outside the cvcpkg" >&2
...
echo "maturin-cp311: cargo $("${CARGO_BIN}" --version) [${CARGO_BIN}]"
echo "maturin-cp311: rustc $("${RUSTC_BIN}" --version) [${RUSTC_BIN}]"
...
if [ -n "${CVC_CARGO_OFFLINE:-}" ]; then
    ...
    echo "maturin-cp311: CARGO_NET_OFFLINE=true — ${CARGO_HOME} must already contain"
    echo "  every crate in Cargo.lock, or cargo will fail rather than fetch."
else
    export CARGO_NET_OFFLINE=false
    echo "maturin-cp311: NOTE — cargo will FETCH crates.io. maturin's sdist ships"
...
shopt -u nullglob
WHEEL="${_wheels[0]:-}"
[ -n "${WHEEL}" ] || { echo "maturin-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "maturin-cp311: built $(basename "${WHEEL}")"
...
[ -n "${SITE_PACKAGES}" ] || {
    echo "maturin-cp311: no site-packages under ${CVC_INSTALL_DIR} after pip install" >&2
    ls -la "${CVC_INSTALL_DIR}" >&2 || true
    exit 1
}
echo "maturin-cp311: staged into ${SITE_PACKAGES}"
...
[ -n "${MATURIN_EXE}" ] || {
    echo "maturin-cp311: no maturin executable staged under ${CVC_INSTALL_DIR}" >&2
    ls -la "${CVC_INSTALL_DIR}"/bin "${CVC_INSTALL_DIR}"/Scripts 2>&1 | head -40 >&2 || true
    exit 1
}
echo "maturin-cp311: staged executable ${MATURIN_EXE}"
"${MATURIN_EXE}" --version
...
echo "maturin-cp311: build + verification complete"
