#!/usr/bin/env bash
# recipes/trove-classifiers-cp313/build.sh — build trove-classifiers 2026.6.1.19 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, calver-cp313
...
echo "trove-classifiers-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trove-classifiers-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trove-classifiers-cp313: built $(basename "${WHEEL}")"
