#!/usr/bin/env bash
# recipes/aiosqlite-cp312/build.sh — build aiosqlite 0.22.1 FROM SOURCE (generated).
#
...
echo "aiosqlite-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "aiosqlite-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "aiosqlite-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import aiosqlite"
