#!/usr/bin/env bash
# recipes/contourpy-cp312/build.sh — build contourpy 1.3.1 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: meson, meson-python-cp312, pybind11-cp312
...
echo "contourpy-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "contourpy-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "contourpy-cp312: built $(basename "${WHEEL}")"
