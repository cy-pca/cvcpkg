#!/usr/bin/env bash
# recipes/platformdirs-cp312/build.sh — build platformdirs 4.11.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: hatch-vcs-cp312, hatchling-cp312, packaging-cp312, pathspec-cp312, pluggy-cp312, trove-classifiers-cp312, calver-cp312
...
echo "platformdirs-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "platformdirs-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "platformdirs-cp312: built $(basename "${WHEEL}")"
