#!/usr/bin/env bash
# recipes/python-multipart-cp313t/build.sh — build python-multipart 0.0.29 FROM SOURCE (generated).
#
...
echo "python-multipart-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "python-multipart-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "python-multipart-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import multipart"
