#!/usr/bin/env bash
# recipes/starlette-cp313t/build.sh — build starlette 1.0.1 FROM SOURCE (generated).
#
...
echo "starlette-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "starlette-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "starlette-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import starlette"
