...
#
# BUILD BACKEND: setuptools-cp312, calver-cp312
...
echo "trove-classifiers-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "trove-classifiers-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "trove-classifiers-cp312: built $(basename "${WHEEL}")"
