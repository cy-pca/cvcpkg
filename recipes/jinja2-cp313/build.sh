#!/usr/bin/env bash
# recipes/jinja2-cp313/build.sh — build jinja2 3.1.4 FROM SOURCE (generated).
#
...
echo "jinja2-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "jinja2-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "jinja2-cp313: built $(basename "${WHEEL}")"
