#!/usr/bin/env bash
# recipes/sympy-cp312/build.sh — build sympy 1.13.3 FROM SOURCE (generated).
#
...
echo "sympy-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "sympy-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "sympy-cp312: built $(basename "${WHEEL}")"
