#!/usr/bin/env bash
# recipes/gast-cp311/build.sh — build gast 0.7.0 FROM SOURCE (generated).
#
...
echo "gast-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gast-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gast-cp311: built $(basename "${WHEEL}")"
