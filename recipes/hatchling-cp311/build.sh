#!/usr/bin/env bash
# recipes/hatchling-cp311/build.sh — build hatchling 1.31.0 FROM SOURCE (generated).
#
...
echo "hatchling-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "hatchling-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "hatchling-cp311: built $(basename "${WHEEL}")"
