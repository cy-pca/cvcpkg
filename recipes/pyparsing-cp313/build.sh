#!/usr/bin/env bash
# recipes/pyparsing-cp313/build.sh — build pyparsing 3.2.1 FROM SOURCE (generated).
#
...
echo "pyparsing-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyparsing-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyparsing-cp313: built $(basename "${WHEEL}")"
