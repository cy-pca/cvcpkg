#!/usr/bin/env bash
# recipes/botocore-cp312/build.sh — build botocore 1.43.49 FROM SOURCE (generated).
#
...
echo "botocore-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "botocore-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "botocore-cp312: built $(basename "${WHEEL}")"
