#!/usr/bin/env bash
# recipes/cppy-cp313/build.sh — build cppy 1.3.1 FROM SOURCE (generated).
#
...
echo "cppy-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cppy-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cppy-cp313: built $(basename "${WHEEL}")"
