#!/usr/bin/env bash
# recipes/iniconfig-cp313/build.sh — build iniconfig 2.3.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313, setuptools-scm-cp313
...
echo "iniconfig-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "iniconfig-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "iniconfig-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import iniconfig"
