#!/usr/bin/env bash
# recipes/anyio-cp312/build.sh — build anyio 4.13.0 FROM SOURCE (generated).
#
...
echo "anyio-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "anyio-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "anyio-cp312: built $(basename "${WHEEL}")"
