#!/usr/bin/env bash
# recipes/pygments-cp313t/build.sh — build pygments 2.20.0 FROM SOURCE (generated).
#
...
echo "pygments-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pygments-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pygments-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pygments"
