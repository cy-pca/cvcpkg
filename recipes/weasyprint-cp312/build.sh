...
#
# BUILD BACKEND: flit-core-cp312, pycparser-cp312
...
echo "weasyprint-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "weasyprint-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "weasyprint-cp312: built $(basename "${WHEEL}")"
