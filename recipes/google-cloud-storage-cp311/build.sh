#!/usr/bin/env bash
# recipes/google-cloud-storage-cp311/build.sh — build google-cloud-storage 3.13.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp311, wheel-cp311
...
echo "google-cloud-storage-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "google-cloud-storage-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "google-cloud-storage-cp311: built $(basename "${WHEEL}")"
