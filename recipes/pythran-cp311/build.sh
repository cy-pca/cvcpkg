#!/usr/bin/env bash
# recipes/pythran-cp311/build.sh — build pythran 0.18.1 FROM SOURCE (generated).
#
...
echo "pythran-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pythran-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pythran-cp311: built $(basename "${WHEEL}")"
