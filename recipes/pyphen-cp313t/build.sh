#!/usr/bin/env bash
# recipes/pyphen-cp313t/build.sh — build pyphen 0.18.1 FROM SOURCE (generated).
#
...
echo "pyphen-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pyphen-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pyphen-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pyphen"
