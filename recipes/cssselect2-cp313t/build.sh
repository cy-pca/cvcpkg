#!/usr/bin/env bash
# recipes/cssselect2-cp313t/build.sh — build cssselect2 0.9.0 FROM SOURCE (generated).
#
...
echo "cssselect2-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "cssselect2-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "cssselect2-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import cssselect2"
