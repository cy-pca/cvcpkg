#!/usr/bin/env bash
# recipes/pluggy-cp313/build.sh — build pluggy 1.6.0 FROM SOURCE (generated).
#
...
echo "pluggy-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pluggy-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "pluggy-cp313: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import pluggy"
