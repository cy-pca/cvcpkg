#!/usr/bin/env bash
# recipes/gymnasium-cp313t/build.sh — build gymnasium 1.3.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: setuptools-cp313t
...
echo "gymnasium-cp313t: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "gymnasium-cp313t: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "gymnasium-cp313t: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import gymnasium; gymnasium.spaces.Box(-1.0, 1.0, (3,))"
