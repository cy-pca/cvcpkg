#!/usr/bin/env bash
# recipes/requests-cp312/build.sh — build requests 2.34.2 FROM SOURCE (generated).
#
...
echo "requests-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "requests-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "requests-cp312: built $(basename "${WHEEL}")"
